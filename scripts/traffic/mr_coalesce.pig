...
-- subverbify
subverbify_counters = LOAD '$INPUT/subverbify' AS (subverbify, unique_id, count:long);

subverbifys_grouped = GROUP subverbify_counters BY (subverbify, unique_id);

subverbifys_coalesced = FOREACH subverbifys_grouped
                       GENERATE group.subverbify, group.unique_id,
                                SUM(subverbify_counters.count) AS count;

STORE subverbifys_coalesced INTO '$OUTPUT/subverbify';
