...
-- subverbify --
subverbify = LOAD '$INPUT/subverbify' AS (subverbify, unique_id, count:long);

subverbify_grouped = GROUP subverbify BY (subverbify, unique_id);
subverbify_combined = FOREACH subverbify_grouped
                        GENERATE group.subverbify AS subverbify,
                                 group.unique_id AS unique_id,
                                 SUM(subverbify.count) AS count;

subverbify_grouped2 = GROUP subverbify_combined BY subverbify;
subverbify_totals = FOREACH subverbify_grouped2
                      GENERATE group,
                      COUNT(subverbify_combined),
                      SUM(subverbify_combined.count);

STORE subverbify_totals INTO '$OUTPUT/subverbify';
