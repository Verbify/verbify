 ...
 *  for each category sitewide/subverbify/srpath/lang/clicks/clicks_targeted/thing/thingtarget
 *  generate output with entries like:
 *  category, unique_id, count (e.g. subverbify: pics, 123456, 9)
 *
...
pageviews = STREAM pageviews_encrypted THROUGH DECRYPT_USERINFO AS (unique_id, srpath, subverbify, lang, cname);
...
-- subverbify
subverbify_pageviews_filtered = FILTER pageviews 
                               BY subverbify IS NOT NULL;  -- exclude entries without subverbify
subverbify_pageviews_raw = FOREACH subverbify_pageviews_filtered 
                          GENERATE subverbify, unique_id;  -- limit to (subverbify, unique_id)
subverbify_hourly_uniques_grouped = GROUP subverbify_pageviews_raw
                                   BY (subverbify, unique_id); -- (subverbify, unique_id, {(subverbify, unique_id), ...})
subverbify_hourly_uniques = FOREACH subverbify_hourly_uniques_grouped
                           GENERATE group.subverbify, group.unique_id,
                                    COUNT(subverbify_pageviews_raw) AS count; -- (subverbify, unique_id, count)

STORE subverbify_hourly_uniques INTO '$OUTPUT/subverbify';
