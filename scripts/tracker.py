...
verbify_domain = config.get('DEFAULT', 'domain')
verbify_domain_prefix = config.get('DEFAULT', 'domain_prefix')
...
def event_redirect():
    ...
    netloc = "%s.%s" % (verbify_domain_prefix, verbify_domain)
...
def event_click():
    ...
    that does not have session information - i.e. served from verbifymedia.com.
    ...
    """
    try:
        session_str = urllib.unquote(request.cookies.get('verbify_session', ''))
