...
relations = {"savehide": ("UserQueryCache", "link"),
             ...
             "moderatorinbox": ("SubverbifyQueryCache", "message"),
...
FOREACH grouped_with_data
    GENERATE FLATTEN(items),
             com.verbify.pig.MAKE_MAP(data.(key, value)) AS data;
"""
...
potential_columns =
FOREACH only_valid
    GENERATE com.verbify.pig.MAKE_ROWKEY(relation, name, thing1_id) AS rowkey,
             com.verbify.pig.MAKE_THING2_FULLNAME(relation, thing2_id) AS colkey,
             timestamp AS value;
"""
...
if False:
    ...
    FOREACH non_null
    GENERATE
        CONCAT('sent.', com.verbify.pig.TO_36(data#'author_id')) AS rowkey,
        com.verbify.pig.MAKE_FULLNAME('message', id) AS colkey,
        timestamp AS value;
    """
    script += store_top_1000_per_rowkey
    compiled = Pig.compile(script)
...
if True:
    ...
    FOREACH comments_with_name GENERATE
        CONCAT(name, CONCAT('.', com.verbify.pig.TO_36(data#'sr_id'))) AS rowkey,
        com.verbify.pig.MAKE_FULLNAME('comment', id) AS colkey,
        timestamp AS value;
    """
    script += store_top_1000_per_rowkey
    compiled = Pig.compile(script)
    bound = compiled.bind({
        ...
        "OUTPUT": "/".join((OUTPUT_ROOT, "SubverbifyQueryCache", "comment")),
    })
    result = bound.runSingle()
