...
###############################################################################
"""Fill in the sildings listing for users.

This listing is stored in get_user_sildings and seen on
/user/<username>/sildings.
...
from v1.lib.db.queries import get_user_sildings
    ...
    while date > earliest_date:
        ...
        for silding in sildings:
            ...
            if fullname in comments and fullname not in already_seen:
                ...
                m.insert(get_user_sildings(silding["user"]), [silding_object])
