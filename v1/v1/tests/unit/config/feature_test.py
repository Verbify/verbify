...
from v1.config.feature.state import FeatureState
from v1.config.feature.world import World
...
class TestFeatureBase(VerbifyTestCase):
    _world = None
...
class TestFeatureBase(VerbifyTestCase):
    ...
    def setUp(self):
        ...
        self.world.current_subverbify = mock.Mock(return_value='')
...
class TestFeature(TestFeatureBase):
    ...
    def test_sodium_enabled(self):
        cfg = {'sodium': True}
        self.world.has_sodium = mock.Mock(return_value=True)
        feature_state = self.world._make_state(cfg)
        self.assertTrue(feature_state.is_enabled(user=gary))

    def test_sodium_disabled(self):
        cfg = {'sodium': True}
        self.world.has_sodium = mock.Mock(return_value=False)
        feature_state = self.world._make_state(cfg)
        self.assertFalse(feature_state.is_enabled(user=gary))
    ...
    def test_subverbify_in(self):
        cfg = {'subverbifys': ['WTF']}
        feature_state = self.world._make_state(cfg)
        self.assertTrue(feature_state.is_enabled(subverbify='wtf'))

        cfg = {'subverbifys': ['wtf']}
        feature_state = self.world._make_state(cfg)
        self.assertTrue(feature_state.is_enabled(subverbify='WTF'))

        cfg = {'subverbifys': ['aww', 'wtf']}
        feature_state = self.world._make_state(cfg)
        self.assertTrue(feature_state.is_enabled(subverbify='wtf'))

    def test_subverbify_not_in(self):
        cfg = {'subverbifys': []}
        feature_state = self.world._make_state(cfg)
        self.assertFalse(feature_state.is_enabled(subverbify='wtf'))

        cfg = {'subverbifys': ['aww', 'wtfoobar']}
        feature_state = self.world._make_state(cfg)
        self.assertFalse(feature_state.is_enabled(subverbify='wtf'))
