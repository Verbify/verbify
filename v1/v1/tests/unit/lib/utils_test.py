...
class UtilsTest(unittest.TestCase):
    ...
    def test_extract_subdomain(self):
        self.assertEquals(
            utils.extract_subdomain('beta.verbify.com', 'verbify.com'),
            'beta')

        self.assertEquals(
            utils.extract_subdomain('beta.verbify.local:8000', 'verbify.local'),
            'beta')

        self.assertEquals(
            utils.extract_subdomain('verbify.com', 'verbify.com'),
            '')

        self.assertEquals(
            utils.extract_subdomain('internet-frontpage.com', 'verbify.com'),
            '')
    ...
    def test_sanitize_url(self):
        ...
        self.assertEquals(
            utils.sanitize_url('https://github.com/verbify/verbify/pull/1302'),
            'https://github.com/verbify/verbify/pull/1302'
        )
...
class TestUrlToThing(unittest.TestCase):
    ...
    def test_subverbify_noslash(self):
        with patch('v1.models.Subverbify') as MockSubverbify:
            MockSubverbify._by_name.return_value = s.Subverbify
            self.assertEqual(
                utils.url_to_thing('http://verbify.local/r/pics'),
                s.Subverbify,
            )

    def test_subverbify(self):
        with patch('v1.models.Subverbify') as MockSubverbify:
            MockSubverbify._by_name.return_value = s.Subverbify
            self.assertEqual(
                utils.url_to_thing('http://verbify.local/r/pics/'),
    ...
    def test_frontpage(self):
        self.assertEqual(
            utils.url_to_thing('http://verbify.local/'),
            None,
        )
