...
from v1.lib.errors import errors, ErrorSet
...
from v1.models import Account, Comment, Link, Message, Subverbify


class ValidatorTests(VerbifyTestCase):
...
class TestVSubmitParent(ValidatorTests):
    def setUp(self):
        ...
        self.autopatch(Subverbify, "_byID", return_value=None)
    ...
    def test_locked_link(self):
        ...
        with patch.object(Subverbify, "can_distinguish", return_value=False):
            result = self.validator.run('fullname', None)
    ...
    def test_locked_link_mod_reply(self):
        link = self._mock_link(locked=True)
        with patch.object(Subverbify, "can_distinguish", return_value=True):
            result = self.validator.run('fullname', None)
...
class TestVSubverbifyName(ValidatorTests):
    def setUp(self):
        # Reset the validator state and errors before every test.
        self.validator = VSubverbifyName(None)
        c.errors = ErrorSet()

    def _test_failure(self, input, error=errors.BAD_SR_NAME):
        super(TestVSubverbifyName, self)._test_failure(input, error)
