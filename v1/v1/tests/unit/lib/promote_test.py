...
from v1.tests import VerbifyTestCase, NonCache


subscriptions_srnames = ["foo", "bar"]
subscriptions = map(lambda srname: Subverbify(name=srname), subscriptions_srnames)
multi_srnames = ["bing", "bat"]
multi_subverbifys = map(lambda srname: Subverbify(name=srname), multi_srnames)
...
naughty_subscriptions = [
    ...
    Subverbify(name=nsfw_srname, over_18=True),
    Subverbify(name=quarantined_srname, quarantine=True),
]
...
class TestSRNamesFromSite(VerbifyTestCase):
    def setUp(self):
    ...
    @patch("v1.models.Subverbify.user_subverbifys")
    ...
    def test_multi_logged_out(self):
        multi = MultiVerbify(path="/user/test/m/multi_test", srs=multi_subverbifys)
    ...
    @patch("v1.models.Subverbify.user_subverbifys")
    def test_quarantined_subscriptions_are_never_included(self, user_subverbifys):
    ...
    @patch("v1.models.Subverbify.user_subverbifys")
    def test_nsfw_subscriptions_arent_included_when_viewing_frontpage(self, user_subverbifys):
        user_subverbifys.return_value = naughty_subscriptions
        srnames = srnames_from_site(self.logged_in, Frontpage)
    ...
    @patch("v1.models.Collection.get_all")
    ...
    @patch("v1.lib.promote.get_nsfw_collections_srnames")
...
class TestPromoteRefunds(unittest.TestCase):
    ...
    @patch('v1.lib.promote.authorize.refund_transaction')
    @patch('v1.lib.promote.PromotionLog.add')
    @patch('v1.lib.promote.queries.unset_underdelivered_campaigns')
    @patch('v1.lib.promote.emailer.refunded_promo')
