...
class TestPadding(unittest.TestCase):
    def test_pad_empty_string(self):
        from v1.lib.tracking import _pad_message
        padded = _pad_message("")
        self.assertEquals(padded, BLOCK_O_PADDING)

    def test_pad_round_string(self):
        from v1.lib.tracking import _pad_message, KEY_SIZE
    ...
    def test_unpad_empty_message(self):
        from v1.lib.tracking import _unpad_message
    ...
    def test_padding_roundtrip(self):
        from v1.lib.tracking import _unpad_message, _pad_message
...
class TestEncryption(unittest.TestCase):
    def test_salt(self):
        from v1.lib.tracking import _make_salt, SALT_SIZE
        self.assertEquals(len(_make_salt()), SALT_SIZE)

    def test_encrypt(self):
        from v1.lib.tracking import _encrypt, SALT_SIZE
        encrypted = _encrypt(
            "a" * SALT_SIZE,
            MESSAGE,
            SECRET,
    ...
    def test_decrypt(self):
        from v1.lib.tracking import _decrypt
