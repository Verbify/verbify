...
from v1.models.link import Link, Comment
...
class LinkMock(Link):
    ...
    def subverbify_slow(self):
        return SubverbifyMock()

    def _commit(self):
        pass

    @classmethod
    @patch('v1.lib.voting.cast_vote')
...
class SubverbifyMock(ThingMock):
    @property
    def lang(self):
        return "en"
...
class TestSubmit(unittest.TestCase):
    def setUp(self):
        from v1.models import (
            LinksByAccount,
            LinksByUrlAndSubverbify,
            SubverbifyParticipationByAccount,
            SubverbifysActiveForFrontPage,
        )

        LinksByAccount.add_link = MagicMock()
        SubverbifyParticipationByAccount.mark_participated = MagicMock()
        SubverbifysActiveForFrontPage.mark_new_post = MagicMock()

        self.links_by_url_add_link = MagicMock()
        LinksByUrlAndSubverbify.add_link = self.links_by_url_add_link
        self.links_by_url_remove_link = MagicMock()
        LinksByUrlAndSubverbify.remove_link = self.links_by_url_remove_link
