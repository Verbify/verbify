...
from v1.lib import eventcollector


__all__ = ['VerbifyTestCase', 'VerbifyControllerTestCase']
...
# v1.lib.captcha which is also in sys.path.  This import ensures
...
try:
    ...
    with patch.object(
        sys, "path",
        [x for x in sys.path if not x.endswith("v1/v1/lib")]
    ):
        from Captcha import Base

from pylons import app_globals as g
from v1.config.middleware import VerbifyApp
...
class VerbifyTestCase(TestCase):
    ...
    def setUp(self):
        ...
        def reset_test_mode(orig_value=VerbifyApp.test_mode):
            VerbifyApp.test_mode = orig_value
        self.addCleanup(reset_test_mode)
        VerbifyApp.test_mode = True
...
class VerbifyControllerTestCase(VerbifyTestCase):
    CONTROLLER = None
    ACTIONS = {}

    def setUp(self):
        super(VerbifyControllerTestCase, self).setUp()
        ...
        # to work properly
        for i, _cls in enumerate((Link, Subverbify, Account)):
            ...
            if not hasattr(_cls, "_type_name"):
                self.autopatch(
                    ...
                    create=True)
        # The same is true for _by_name on Subverbify and Account
        self.subverbify_by_name = self.autopatch(Subverbify, "_by_name")
