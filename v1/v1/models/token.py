...
class OAuth2Scope:
    scope_info = {
        ...
        "cverbifys": {
            "id": "cverbifys",
            "name": _("Spend verbify sodium cverbifys"),
            "description": _("Spend my verbify sodium cverbifys on giving "
                "sodium to other users."),
        },
        "edit": {
            "id": "edit",
            "name": _("Edit Posts"),
        ...
        "flair": {
            ...
            "description": _("Select my subverbify flair. "
        ...
        "identity": {
            "id": "identity",
            "name": _("My Identity"),
            "description": _("Access my verbify username and signup date."),
        },
        "modcontributors": {
            "id": "modcontributors",
            ...
            "description": _(
                ...
                "ban/unban or mute/unmute users from subverbifys I moderate."
            ),
        },
        "modflair": {
            "id": "modflair",
            ...
            "description": _(
                "Manage and assign flair in subverbifys I moderate."),
        },
        "modposts": {
            "id": "modposts",
            ...
            "description": _(
                ...
                " in subverbifys I moderate."),
        },
        "modconfig": {
            "id": "modconfig",
            "name": _("Moderate Subverbify Configuration"),
            "description": _(
                "Manage the configuration, sidebar, and CSS"
                " of subverbifys I moderate."),
        },
        "modlog": {
            "id": "modlog",
            "name": _("Moderation Log"),
            "description": _(
                "Access the moderation log in subverbifys I moderate."),
        },
        "modothers": {
            "id": "modothers",
            "name": _("Invite or remove other moderators"),
            "description": _(
                "Invite or remove other moderators from subverbifys I moderate."
            ),
        },
        "modself": {
            "id": "modself",
            "name": _("Make changes to your subverbify moderator "
                      "and contributor status"),
            "description": _(
                "Accept invitations to moderate a subverbify. Remove myself as "
                "a moderator or contributor of subverbifys I moderate or "
                "contribute to."
            ),
        },
        "modtraffic": {
            "id": "modtraffic",
            "name": _("Subverbify Traffic"),
            "description": _("Access traffic stats in subverbifys I moderate."),
        },
        "modwiki": {
            "id": "modwiki",
        ...
        "mysubverbifys": {
            "id": "mysubverbifys",
            "name": _("My Subverbifys"),
            "description": _(
                "Access the list of subverbifys I moderate, contribute to,"
                " and subscribe to."),
        },
        "privatemessages": {
            "id": "privatemessages",
        ...
        "subscribe": {
            ...
            "description": _('Manage my subverbify subscriptions. Manage '
    ...
    def __init__(self, scope_str=None, subverbifys=None, scopes=None):
        if scope_str:
            self._parse_scope_str(scope_str)
        elif subverbifys is not None or scopes is not None:
            self.subverbify_only = bool(subverbifys)
            self.subverbifys = subverbifys
            self.scopes = scopes
        else:
            self.subverbify_only = False
            self.subverbifys = set()
            self.scopes = set()

    def _parse_scope_str(self, scope_str):
        srs, sep, scopes = scope_str.rpartition(':')
        if sep:
            self.subverbify_only = True
            self.subverbifys = set(srs.split('+'))
        else:
            self.subverbify_only = False
            self.subverbifys = set()
    ...
    def __str__(self):
        if self.subverbify_only:
            sr_part = '+'.join(sorted(self.subverbifys)) + ':'
        else:
            sr_part = ''
        return sr_part + ' '.join(sorted(self.scopes))

    def has_access(self, subverbify, required_scopes):
        if self.FULL_ACCESS in self.scopes:
            return True
        if self.subverbify_only and subverbify not in self.subverbifys:
            return False
    ...
    def merge_scopes(cls, scopes):
        """Return a by-subverbify dict representing merged OAuth2Scopes.

        Takes an iterable of OAuth2Scopes. For each of those,
        if it defines scopes on multiple subverbifys, it is split
        into one OAuth2Scope per subverbify. If multiple passed in
        OAuth2Scopes reference the same scopes, they'll be combined.

        """
        merged = {}
        for scope in scopes:
            srs = scope.subverbifys if scope.subverbify_only else (None,)
            for sr in srs:
                if sr in merged:
                    merged[sr].scopes.update(scope.scopes)
                else:
                    new_scope = cls()
                    new_scope.subverbifys = {sr}
                    new_scope.scopes = scope.scopes
                    if sr is not None:
                        new_scope.subverbify_only = True
                    merged[sr] = new_scope
        return merged
