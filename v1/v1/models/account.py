...
from v1.lib import amqp, filters, hooks
from v1.lib.db.thing import Thing, Relation, NotFound
from v1.lib.db.operators import lower
from v1.lib.db.userrel import UserRel
...
from v1.models.bans import TempTimeout
from v1.models.last_modified import LastModified
...
class Account(Thing):
    ...
    _defaults = dict(pref_numsites = 25,
                     ...
                     pref_media = 'subverbify',
                     pref_media_preview = 'subverbify',
                     wiki_override = None,
                     email = "",
    ...
    def has_interacted_with(self, sr):
        try:
            r = SubverbifyParticipationByAccount.fast_query(self, [sr])
    ...
    def incr_karma(self, kind, sr, amt):
        ...
        # having a massive number of subverbify-karma attributes
        if self.disable_karma:
            return

        if sr.name.startswith('_'):
            g.log.info("Ignoring karma increase for subverbify %r" % (sr.name,))
            return
    ...
    def all_karmas(self, include_old=True):
        """Get all of the user's subverbify-specific karma totals.

        Returns an OrderedDict keyed on subverbify name and containing
    ...
    @property
    def can_create_subverbify(self):
        hook = hooks.get_hook("account.can_create_subverbify")
    ...
    def is_mutable(self, subverbify):
        # Don't allow muting of other mods in the subverbify
        if subverbify.is_moderator(self):
            return False

        # Don't allow muting of u/verbify or u/AutoModerator
        return not (self == self.system_user() or
            self == self.automoderator_user())

    # Used on the sodiummember version of /prefs/friends
    ...
    def _banned(self, value):
        if value and not self._banned:
            ...
            from v1.models.token import OAuth2AccessToken
            from v1.models.token import OAuth2RefreshToken
    ...
    @property
    def subverbifys(self):
        from subverbify import Subverbify
        return Subverbify.user_subverbifys(self)
    ...
    def set_subverbify_style(self, sr, use_style):
        if hasattr(sr, '_id'):
    ...
    def can_flair_in_sr(self, user, sr):
        """Return whether a user can set this one's flair in a subverbify."""
    ...
    def set_flair(self, subverbify, text=None, css_class=None, set_by=None,
            log_details="edit"):
        log_details = "flair_%s" % log_details
        if not text and not css_class:
            # set to None instead of potentially empty strings
            text = css_class = None
            subverbify.remove_flair(self)
            log_details = "flair_delete"
        elif not subverbify.is_flair(self):
            subverbify.add_flair(self)

        setattr(self, 'flair_%s_text' % subverbify._id, text)
        setattr(self, 'flair_%s_css_class' % subverbify._id, css_class)
        self._commit()

        if set_by and set_by != self:
            ModAction.create(subverbify, set_by, action='editflair',
    ...
    @property
    def has_sodium_subscription(self):
        return bool(getattr(self, 'sodium_subscr_id', None))

    @property
    def has_paypal_subscription(self):
        return (self.has_sodium_subscription and
                not self.sodium_subscr_id.startswith('cus_'))

    @property
    def has_stripe_subscription(self):
        return (self.has_sodium_subscription and
                self.sodium_subscr_id.startswith('cus_'))

    @property
    def sodium_will_autorenew(self):
        return (self.has_sodium_subscription or
                (self.pref_cverbify_autorenew and self.sodium_cverbifys > 0))
...
class BlockedSubverbifysByAccount(tdb_cassandra.DenormalizedRelation):
    _use_db = True
    _last_modified_name = 'block_subverbify'
...
def deleted_account_cleanup(data):
    ...
    from v1.models.flair import Flair
    from v1.models.token import OAuth2Client
    ...
    for account_id36 in data.itervalues():
        ...
        for rel_type, description in rel_removal_descriptions.iteritems():
            try:
                ids_fn = getattr(Subverbify, "reverse_%s_ids" % rel_type)
                sr_ids = ids_fn(account)

                sr_names = []
                srs = Subverbify._byID(sr_ids, data=True, return_dict=False)
                for subverbify in srs:
                    remove_fn = getattr(subverbify, "remove_" + rel_type)
                    remove_fn(account)
                    sr_names.append(subverbify.name)
...
class SubverbifyParticipationByAccount(tdb_cassandra.DenormalizedRelation):
    _use_db = True
    ...
    @classmethod
    def mark_participated(cls, account, subverbify):
        cls.create(account, [subverbify])


class QuarantinedSubverbifyOptInsByAccount(tdb_cassandra.DenormalizedRelation):
    _use_db = True
    _last_modified_name = 'QuarantineSubverbifyOptin'
    _read_consistency_level = tdb_cassandra.CL.QUORUM
    _write_consistency_level = tdb_cassandra.CL.QUORUM
    _extra_schema_creation_args = {
        "key_validation_class": ASCII_TYPE,
    ...
    @classmethod
    def opt_in(cls, account, subverbify):
        if subverbify.quarantine:
            cls.create(account, subverbify)

    @classmethod
    def opt_out(cls, account, subverbify):
        if subverbify.is_subscriber(account):
            subverbify.remove_subscriber(account)
        cls.destroy(account, subverbify)

    @classmethod
    def is_opted_in(cls, user, subverbify):
        try:
            r = cls.fast_query(user, [subverbify])
        except tdb_cassandra.NotFound:
            return False
        return (user, subverbify) in r
