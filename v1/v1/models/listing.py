...
from vote import *
from report import *
from subverbify import DefaultSR, AllSR, Frontpage, Subverbify
from pylons import i18n, request
from pylons import app_globals as g
from pylons.i18n import _

from v1.config import feature
from v1.lib.wrapped import Wrapped, CachedVariable
from v1.lib import utils
from v1.lib.db import operators
from v1.models import rules
...
class Listing(object):
    ...
    def get_items(self, *a, **kw):
        ...
        from v1.lib.template_helpers import replace_render
...
class FriendListing(UserListing):
    ...
    @property
    def _class(self):
        return '' if not c.user.sodium else 'sodium-accent rounded'
...
class BannedListing(UserListing):
    ...
    def __init__(self, builder, show_jump_to=False, show_not_found=False,
        ...
        self.rules = rules.SubverbifyRules.get_rules(c.site)
    ...
    def title(self):
        return _("users banned from"
                 " /r/%(subverbify)s") % dict(subverbify=c.site.name)
...
class WikiMayContributeListing(ContributorListing):
    ...
    def title(self):
        return _("approved wiki contributors"
                 " for /r/%(subverbify)s") % dict(subverbify=c.site.name)
...
class InvitedModListing(UserListing):
    ...
    remove_self_title = _('you are a moderator of this subverbify. %(action)s')

    @property
    def permissions_form(self):
        from v1.lib.permissions import ModeratorPermissionSet
        from v1.lib.pages import ModeratorPermissions
    ...
    def title(self):
        return _("invited moderators for"
                 " %(subverbify)s") % dict(subverbify=c.site.name)
...
class ModListing(InvitedModListing):
    ...
    @property
    def title(self):
        return _("moderators of /r/%(subverbify)s") % dict(subverbify=c.site.name)
...
class SearchListing(LinkListing):
    ...
    def listing(self, legacy_render_class=False, *args, **kwargs):
        ...
        if hasattr(self.builder, 'subverbify_facets'):
            self.subverbify_facets = self.builder.subverbify_facets
