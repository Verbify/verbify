...
from v1.lib.db.thing import NotFound
from v1.lib.errors import MessageError
from v1.lib.utils import tup, fetch_things2
from v1.lib.filters import websafe
from v1.lib.hooks import HookRegistrar
...
from v1.models.award import Award
from v1.models.sodium import append_random_bottlecap_phrase, cverbifys_lock
from v1.models.token import AwardClaimToken
...
class AdminTools(object):
    ...
    def adjust_sodium_expiration(self, account, days=0, months=0, years=0):
        ...
        existing_expiration = getattr(account, "sodium_expiration", None)
        if existing_expiration is None or existing_expiration < now:
            existing_expiration = now
        account.sodium_expiration = existing_expiration + timedelta(days)
        
        if account.sodium_expiration > now and not account.sodium:
            self.ensodiumen(account)
        elif account.sodium_expiration <= now and account.sodium:
            self.desodiumen(account)

        account._commit()     

    def ensodiumen(self, account):
        now = datetime.now(g.display_tz)
        account.sodium = True
        description = "Since " + now.strftime("%B %Y")
        
        trophy = Award.give_if_needed("verbify_sodium", account,
    ...
    def desodiumen(self, account):
        Award.take_away("verbify_sodium", account)
        account.sodium = False
        account._commit()
...
def cancel_subscription(subscr_id):
    q = Account._query(Account.c.sodium_subscr_id == subscr_id, data=True)
    l = list(q)
    if len(l) != 1:
        g.log.warning("Found %d matches for canceled subscription %s"
                      % (len(l), subscr_id))
    for account in l:
        account.sodium_subscr_id = None
        account._commit()
        g.log.info("%s canceled their recurring subscription %s" %
                   (account.name, subscr_id))

def all_sodium_users():
    q = Account._query(Account.c.sodium == True, Account.c._spam == (True, False),
                       data=True, sort="_id")
...
def accountid_from_subscription(subscr_id):
    ...
    q = Account._query(Account.c.sodium_subscr_id == subscr_id,
...
def update_sodium_users():
    now = datetime.now(g.display_tz)
    warning_days = 3
    renew_msg = _("[Click here for details on how to set up an "
                  "automatically-renewing subscription or to renew.]"
                  "(/sodium) If you have any thoughts, complaints, "
                  "rants, suggestions about verbify sodium, please write "
                  "to us at %(sodium_email)s. Your feedback would be "
                  "much appreciated.\n\nThank you for your past "
                  "patronage.") % {'sodium_email': g.sodiumsupport_email}

    for account in all_sodium_users():
        days_left = (account.sodium_expiration - now).days
        if days_left < 0:
            if account.pref_cverbify_autorenew:
                with cverbifys_lock(account):
                    if account.sodium_cverbifys > 0:
                        admintools.adjust_sodium_expiration(account, days=31)
                        account.sodium_cverbifys -= 1
                        account._commit()
                        continue

            admintools.desodiumen(account)

            subject = _("Your verbify sodium subscription has expired.")
            message = _("Your subscription to verbify sodium has expired.")
            message += "\n\n" + renew_msg
            message = append_random_bottlecap_phrase(message)

            send_system_message(account, subject, message,
                                distinguished='sodium-auto')
        elif days_left <= warning_days and not account.sodium_will_autorenew:
            hc_key = "sodium_expiration_notice-" + account.name
            already_warned = g.hardcache.get(hc_key)
            if not already_warned:
                g.hardcache.set(hc_key, True, 86400 * (warning_days + 1))
                
                subject = _("Your verbify sodium subscription is about to "
                            "expire!")
                message = _("Your subscription to verbify sodium will be "
                            "expiring soon.")
                message += "\n\n" + renew_msg
                message = append_random_bottlecap_phrase(message)
...
def wiki_template(template_slug, sr=None):
    """Pull content from a subverbify's wiki page for internal use."""
    if not sr:
        try:
            sr = Subverbify._by_name(g.default_sr)
        except NotFound:
            return None
...
def send_system_message(user, subject, body, system_user=None,
    ...
    try:
        ...
        raise MessageError('verbify_inbox')


if config['v1.import_private']:
    from v1admin.models.admintools import *
