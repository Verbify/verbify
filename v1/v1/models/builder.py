...
from v1.config.extensions import API_TYPES, RSS_TYPES
from v1.lib import hooks
from v1.lib.comment_tree import (
    conversation,
    get_comment_scores,
    moderator_messages,
    sr_conversation,
...
from v1.lib.db import operators, tdb_cassandra
from v1.lib.filters import _force_unicode
from v1.lib.jsontemplates import get_trimmed_sr_dicts
...
class Builder(object):
    ...
    def wrap_items(self, items):
        ...
        friend_rels = user.friend_rels() if user and user.sodium else {}

        subverbifys = Subverbify.load_subverbifys(items, stale=self.stale)
        can_ban_set = set()

        if user:
            for sr_id, sr in subverbifys.iteritems():
                if sr.can_ban(user):
        ...
        for item in items:
            ...
            if hasattr(item, "sr_id") and item.sr_id is not None:
                w.subverbify = subverbifys[item.sr_id]
            ...
            if w.distinguished == 'moderator':
                add_moderator_distinguish(distinguish_attribs_list, w.subverbify)
            ...
            # if the user can ban things on a given subverbify, or an
...
class QueryBuilder(Builder):
    ...
    def get_items(self):
        ...
        if getattr(self, "sr_detail", False) and c.render_style in API_TYPES:
            items_by_subverbify = defaultdict(list)
            for item in items:
                if isinstance(item.lookups[0], Link):
                    items_by_subverbify[item.subverbify].append(item)

            srs = items_by_subverbify.keys()
            sr_dicts = get_trimmed_sr_dicts(srs, c.user)

            for sr, sr_items in items_by_subverbify.iteritems():
...
class SearchBuilder(IDBuilder):
    ...
    def init_query(self):
        ...
        self.subverbify_facets = self.results.subverbify_facets
    ...
    def keep_item(self, item):
        ...
        if item._spam or item._deleted:
            return False
        # If checking (wrapped) links, filter out banned subverbifys
        elif hasattr(item, 'subverbify') and item.subverbify.spammy():
            return False
        elif (hasattr(item, 'subverbify') and
              not c.user_is_admin and
              not item.subverbify.is_exposed(user)):
            return False
        elif (self.skip_deleted_authors and
              getattr(item, "author", None) and item.author._deleted):
            return False
        elif isinstance(item.lookups[0], Subverbify) and not item.is_exposed(user):
            return False
        ...
        is_nsfw = (item.over_18 or
            (hasattr(item, 'subverbify') and item.subverbify.over_18))
...
class WikiRevisionBuilder(QueryBuilder):
    ...
    def wrap_items(self, items):
        from v1.lib.validator.wiki import this_may_revise
        types = {}
        wrapped = []
    ...
    def keep_item(self, item):
        from v1.lib.validator.wiki import may_view
...
class CommentOrderer(CommentOrdererBase):
    ...
    def write_cache(cls, link, sort, timer):
        comment_orderer = cls(link, sort,
            max_comments=g.max_comments_sodium,
...
class MessageBuilder(Builder):
    ...
    def _viewable_message(self, m):
        ...
        subverbify = getattr(m, "subverbify", None)
        if subverbify and subverbify.is_moderator_with_perms(c.user, 'mail'):
            return True
...
class ModeratorMessageBuilder(MessageBuilder):
    ...
    def get_tree(self):
        if self.parent:
            sr = Subverbify._byID(self.parent.sr_id)
            return sr_conversation(sr, self.parent)
        sr_ids = Subverbify.reverse_moderator_ids(self.user)
        return moderator_messages(sr_ids)


class MultiverbifyMessageBuilder(MessageBuilder):
...
class SrMessageBuilder(MessageBuilder):
    ...
    def get_tree(self):
        ...
        return subverbify_messages(self.sr)
