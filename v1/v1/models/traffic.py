...
###############################################################################
"""
These models represent the traffic statistics stored for subverbifys and
...
from v1.lib.utils import timedelta_by_name, tup
...
def campaign_history(cls, codenames, start, stop):
    ...
    q = (Session.query(cls)
                ...
                .filter(cls.date.in_(time_points))
                .order_by(cls.date))
    return [(r.date, r.codename, r.subverbify, (r.unique_count,
                                               r.pageview_count))
            for r in q.all()]
...
class PageviewsBySubverbifyAndPath(Base):
    """Pageviews within a subverbify with action included.

    `srpath` is the subverbify name, a dash, then the controller method called
    to render the page the user viewed. e.g. verbify.com-GET_listing. This is
    useful to determine how many pageviews in a subverbify are on listing pages,
...
class SubscriptionsBySubverbify(Base):
    """Subscription statistics for subverbifys.
