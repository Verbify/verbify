...
function subscribe(verbify_name) {
    return function() { 
        if (r.config.logged) {
            if (r.config.cur_site == verbify_name) {
                $('body').addClass('subscriber');
            }
            $.things(verbify_name).find(".entry").addClass("likes");
            $.request("subscribe", {sr: verbify_name, action: "sub"});
            r.analytics.fireUITrackingPixel("sub", verbify_name, {"has_subd": r.config.has_subscribed})
        }
    };
};

function unsubscribe(verbify_name) {
    return function() { 
        if (r.config.logged) {
            if (r.config.cur_site == verbify_name) {
                $('body').removeClass('subscriber');
            }
            $.things(verbify_name).find(".entry").removeClass("likes");
            $.request("subscribe", {sr: verbify_name, action: "unsub"});
            r.analytics.fireUITrackingPixel("unsub", verbify_name)
        }
    };
};

function quarantine_optout(subverbify_name) {
    return function() {
        if (r.config.logged) {
            $.request("quarantine_optout", {sr: subverbify_name});
            $.redirect("/");
        }
    };
};
...
function highlight_verbify(item) {
...
$(function() {
        ...
        $('.search-result-listing')
          .find('.search-title, .search-link, .search-subverbify-link, .search-result-body')
          .highlight(query);
        ...
        $(".verbify-themes .theme").click(function() {
