$(function() {
  var sessionStorageKey = 'subverbify-rules';
  ...
  function init() {
    var subverbifyRulesTemplate = _getTemplate('subverbify-rules-report-template');
    var subverbifyDefaultTemplate = _getTemplate('subverbify-default-report-template');
    var verbifyTemplate = _getTemplate('verbify-report-template');
    var reasonTemplate = _getTemplate('report-reason-template');

    templates = {
      subverbifyRules: function(data) {
        var rulesStr = data.rules.map(reasonTemplate).join('\n');
        var formStr = subverbifyRulesTemplate(data);
      ...
      subverbifyDefault: function(data) {
        var formStr = subverbifyDefaultTemplate(data);
        var formEl = $.parseHTML(formStr);
        return formEl;
      },

      verbify: function(data) {
        var formStr = verbifyTemplate(data);
        var formEl = $.parseHTML(formStr);
        return formEl;
      },
    };
  ...
  function renderFromTemplate(data, thingType) {
    var hasSubverbify = !!data.sr_name;
    var hasRules = data.rules && data.rules.length > 0;
    var template;
    var templateData;

    if (!hasSubverbify) {
      template = templates.verbify;
    } else if (hasRules) {
      template = templates.subverbifyRules;
    } else {
      template = templates.subverbifyDefault;
    }
  ...
  function openReportForm(e) {
    ...
    var srFullname = $thing.data('subverbify-fullname');
    ...
    if (useHtmlAPI) {
      ...
      // if no subverbify, render the verbify form (never needs to hit API)
      var formData = { fullname: attrs.thing };
      var form = renderFromTemplate(formData, thingType);
      showForm($reportForm, form);
    } else if (srFullname in cachedRules) {
      // render from cached if available (only needs to hit API once per subverbify)
  ...
  r.hooks.get('setup').register(function() {
    ...
    // temp fix for broken subverbify-report-form
    $('div.content').on('submit', '.subverbify-report-form', function(e) {
