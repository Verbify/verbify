...
var exports = r.sponsored = {
    ...
    setup_collection_selector: function() {
        ...
        var $subverbifyList = $('.collection-subverbify-list ul');
        var $collectionLabel = $('.collection-subverbify-list .collection-label');
        var $frontpageLabel = $('.collection-subverbify-list .frontpage-label');

        var subverbifyNameTemplate = _.template('<% _.each(sr_names, function(name) { %>'
            + ' <li><%= name %></li> <% }); %>');
        var render_subverbify_list = _.bind(function(collection) {
            if (collection === 'none' ||
                    typeof this.collectionsByName[collection] === 'undefined') {
                return '';
            }
            else {
                return subverbifyNameTemplate(this.collectionsByName[collection]);
            }
        }, this);
        ...
        this.collapse_collection_selector = function collapse_widget() {
            ...
            var subverbifyListItems = render_subverbify_list(val);
            $subverbifyList.html(subverbifyListItems);
            if (val === 'none') {
    ...
    setup_collections: function(collections, defaultValue) {
        ...
        this.collections = [{
            name: 'none',
            sr_names: null,
            description: 'influencers on verbify’s highest trafficking page',
        }].concat(collections || []);
        ...
        var template = _.template('<label class="form-group">'
          ...
          + '    <span class="label"><% print(name === \'none\' ? \'Verbify front page\' : name) %></span>'           + '    <small class="description"><%= description %></small>'
          + '  </div>'
          + '</label>');
    ...
    get_cpm: function($form) {
        ...
        var isSubverbify = $form.find('input[name="targeting"][value="one"]').is(':checked');
        var collectionVal = $form.find('input[name="collection"]:checked').val();
        var isFrontpage = !isSubverbify && collectionVal === 'none';
        var isCollection = !isSubverbify && !isFrontpage;
        var sr = isSubverbify ? $form.find('*[name="sr"]').val() : '';
        ...
        if (isFrontpage) {
            ...
            prices.push(this.priceDict.SUBVERBIFY[sr] || this.priceDict.SUBVERBIFY_DEFAULT);
        }
    ...
    get_targeting: function($form) {
        var isSubverbify = $form.find('input[name="targeting"][value="one"]').is(':checked'),
            ...
            isFrontpage = !isSubverbify && collectionVal === 'none',
            isCollection = !isSubverbify && !isFrontpage,
            type = isFrontpage ? 'frontpage' : isCollection ? 'collection' : 'subverbify',
            sr = isSubverbify ? $form.find('*[name="sr"]').val() : '',
            ...
            isValid = isFrontpage || (isSubverbify && sr) || (isCollection && collection);
    ...
    get_campaigns: function($list, $form) {
        ...
        var fixedCPMSubverbifys = {};
        var totalFixedCPMBudgetDollars = 0;
        var auctionCampaigns = 0;
        var auctionSubverbifys = {};
        var totalAuctionBudgetDollars = 0;
        var totalImpressions = 0;

        function mapSubverbify(name, subverbifys) {
            subverbifys[name] = 1;
        }

        function getSubverbifysByCollection(name) {
            return collections[name] && collections[name].sr_names || null;
        }

        function mapCollection(name, subverbifys) {
            var subverbifyNames = getSubverbifysByCollection(name);
            if (subverbifyNames) {
                _.each(subverbifyNames, function(subverbifyName) {
                    mapSubverbify(subverbifyName, subverbifys);
                });
            }
        }
        ...
        _.each(campaignRows, function(row) {
            ...
            var mappingFunction = isCollection ? mapCollection : mapSubverbify;
            var budget = parseFloat(data.total_budget_dollars, 10);

            if (data.is_auction === 'True') {
                auctionCampaigns++;
                mappingFunction(data.targeting, auctionSubverbifys);
                totalAuctionBudgetDollars += budget;
            } else {
                fixedCPMCampaigns++;
                mappingFunction(data.targeting, fixedCPMSubverbifys);
        ...
        return {
            ...
            fixedCPMSubverbifys: fixedCPMSubverbifys,
            auctionSubverbifys: _.keys(auctionSubverbifys),
            fixedCPMSubverbifys: _.keys(fixedCPMSubverbifys),
    ...
    auction_dashboard_help_template: _.template('<p>there '
        ...
        + '<%= auctionSubverbifys.length %> subverbify'
        + '<% auctionSubverbifys.length > 1 && print("s") %></p>'),
    ...
    fixed_cpm_dashboard_help_template: _.template('<p>there '
        ...
        + '<%= fixedCPMSubverbifys.length %> subverbify'
        + '<% fixedCPMSubverbifys.length > 1 && print("s") %>, amounting to a '
    ...
    subverbify_targeting: function() {
        $('.subverbify-targeting').find('*[name="sr"]').prop("disabled", false).end().slideDown();
        $('.collection-targeting').find('*[name="collection"]').prop("disabled", true).end().slideUp();
        this.render()
    },

    collection_targeting: function() {
        $('.subverbify-targeting').find('*[name="sr"]').prop("disabled", true).end().slideUp();
    ...
    submit_inventory_form: function() {
        ...
        else if (targeting.type === 'subverbify') {
        ...
        $campaign_row.fadeOut(function() {
            ...
            if (targeting && !isCollection) {
                ...
                campaign.find('*[name="sr"]').val(targeting).prop("disabled", false).end()
                    .find(".subverbify-targeting").show();
                ...
                campaign.find('*[name="sr"]').val("").prop("disabled", true).end()
                    .find(".subverbify-targeting").hide();
...
function create_campaign() {
    ...
    cancel_edit(function() {
            ...
            $("#campaign")
                ...
                .find(".subverbify-targeting").hide().end()
