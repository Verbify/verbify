...
!function(r, undefined) {
  ...
  /** How long to wait for the subverbify rules to load. @const */
  var RULES_TIMEOUT_MS = 10000;


  /** This looks like a subverbify name. @const */
  var SUBVERBIFY = /^(?:#|\/?r\/)(.*)/;
  ...
  var mc = r.messagecompose = {
    ...
    init: function() {
      ...
      if (mc.dom.$form.length) {
        ...
        // submission (post_form in verbify.js) so that it does't interfere with the
        ...
        if (mc.dom.$to.length) {
          ...
          // subverbify, so we have to load them up front.
          mc.onToUpdate();
        }
    ...
    /**
     * Loads the rules for the given subverbify.
     * Returns a promise that resolves with the rules JSON object or rejects if the
     * subverbify doesn't exist or otherwise fails to load.
     *
     * @private
     * @param {string} sr The name of the subverbify.
     * @return {Promise<{
     *     sr_name: string,
     ...
     *  }>}
     */
    loadSubverbifyRules: function(sr) {
      var url = '/r/' + sr + '/about/rules.json';

      // If we're already loading this subverbify, continue, otherwise, abort
    ...
    /**
     * Renders the rules of the subverbify into the subject field, while maintaining
     * any user entered value.
     *
     * @private
     * @param {!Object} rulesJson
     */
    renderSubverbifySubject: function(rulesJson) {
      ...
      if (rules.length) {
        ...
        if (_(rules).contains(mc.dom.$subject.val())) {
          ...
          // the subverbify is not verbify.com.
          //
          ...
          // To verbify.com. Select Blank by default to encourage consistent subjects.
          mc.dom.$blank.show();
    ...
    /**
     * Changes the state of the form to be correct for a non-subverbify recipient.
     * @private
     */
    ...
    onToUpdate: function() {
      ...
      // Is this probably a subverbify?
      var m = SUBVERBIFY.exec(toValue);
      if (m) {
        mc.loadSubverbifyRules(m[1])
          .then(mc.renderSubverbifySubject, mc.renderGeneralSubject);
      } else {
  ...
  r.hooks.get('verbify-init').register(r.messagecompose.init);
