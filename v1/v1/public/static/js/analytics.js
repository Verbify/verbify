r.analytics = {
  init: function() {
    // these guys are relying on the custom 'onshow' from jquery.verbify.js
    $(document).delegate(
    ...
    if (r.config.user_id) {
      ...
      var tracker = new verbifylib.Tracker();
  ...
  fireFunnelEvent: function(category, action, options, callback) {
    ...
    // if it's for Sodium tracking and we have new _ga available
  ...
  expandoEvent: function(actionName, targetData) {
    ...
    if ('subverbifyName' in targetData) {
      payload['sr_name'] = targetData.subverbifyName;
    }

    if ('subverbifyFullname' in targetData) {
      payload['sr_id'] = r.utils.fullnameToId(targetData.subverbifyFullname);
    }
...
r.analytics.breadcrumbs = {
  selector: '.thing, .side, .sr-list, .srdrop, .tagline, .md, .organic-listing, .gadget, .sr-interest-bar, .trending-subverbifys, a, button, input',
  maxLength: 3,
  sendLength: 2,
