/* The verbify extension for jquery.  This file is intended to store
...
$.unsafe = function(text) {
    /* inverts websafe filtering of verbify app. */
    if(typeof(text) == "string") {
        text = text.replace(/&quot;/g, '"')
            .replace(/&gt;/g, ">").replace(/&lt;/g, "<")
            .replace(/&amp;/g, "&");
...
/* customized requests (formerly verbifyRequest) */
...
$.request = function(op, parameters, worker_in, block, type, 
    ...
    /* 
       Uniquitous verbify AJAX poster.  Automatically addes
       handleResponse(action) worker to deal with the API result.  The
       current subverbify (r.config.post_site) and the user's modhash
    ...
    /* set the subverbify name if there is one */
...
$.fn.thing_id = function() {
    /* Returns the (verbify) ID of the current element's thing */
...
$.apply_stylesheet_url = function(cssUrl, srStyleEnabled) {
  var sheetTitle = 'applied_subverbify_stylesheet';
...
/* old verbify-specific cookie functions */
