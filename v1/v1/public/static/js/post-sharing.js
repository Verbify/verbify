!function(r) {
  ...
  var postSharingTemplate = _.template(
    ...
    '<div class="post-sharing-email-form post-sharing-form" ref="$emailForm">' +
      '<p class="post-sharing-label">' +
        ...
        '<span ref="$emailFormPMLabel">' + _.escape(r._('Share via private message on verbify as %(username)s').format({ username: r.config.logged })) + '</span>' +
      '</p>' +
      '<div class="c-form-group">' +
        '<input class="post-sharing-recipient-input c-form-control" ' +
               ...
               'data-placeholder-verbify-pm="username">' +
  ...
  r.ui.PostSharing = Backbone.View.extend({
    ...
    shareToFacebook: function() {
      var verbifyUrl = this.getShareLink('facebook');
    ...
    shareToTwitter: function() {
      var verbifyUrl = this.getShareLink('twitter');
      var twitterHandle = 'verbify'
    ...
    shareToTumblr: function() {
      var verbifyUrl = this.getShareLink('tumblr');
      var title = this.thingData.title;
      var shareParams = {
        canonicalUrl: verbifyUrl,
        posttype: 'link',
        title: title,
      };
    ...
    render: function() {
      ...
      if ('selectedOption' in changed) {
          ...
          this.refs.$emailFormPMLabel.toggle(selectedOption === 'verbify-pm');

          this.refs.$shareTo.attr('placeholder', this.refs.$shareTo.data('placeholder-' + selectedOption));

        if (selectedOption === 'email' || selectedOption === 'verbify-pm') {
  ...
  $(function() {
    $('body').on('click', '.post-sharing-button', function(e) {
      ...
      if (r.config.logged && !r.config.user_in_timeout) {
        shareOptions.push({
          name: 'verbify-pm',
          tooltip: r._('Private Message a Friend on Verbify'),
        });
      }
