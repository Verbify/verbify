...
from v1.config.extensions import is_api, set_extension
from v1.lib import (
...
from v1.lib.base import BaseController, abort
from v1.lib.cookies import (
...
from v1.lib.filters import _force_utf8, _force_unicode, scriptsafe_dumps
from v1.lib.loid import LoId
from v1.lib.require import RequirementException, require, require_split
from v1.lib.strings import strings
from v1.lib.template_helpers import add_sr, JSPreload
from v1.lib.tracking import encrypt, decrypt, get_pageview_pixel_url
...
def set_subverbify():
    ...
    sr_name = request.environ.get("subverbify", request.params.get('r'))
    ...
    if not sr_name:
        ...
        if cname:
            sr = Subverbify._by_domain(cname) or Frontpage
            domain = g.domain
            if g.domain_prefix:
        ...
        name_filter = lambda name: Subverbify.is_valid_name(name,
            allow_language_srs=True)
        sr_names = filter(name_filter, sr_name.split('+'))
        srs = Subverbify._by_name(sr_names, stale=can_stale).values()
        if All in srs:
            c.site = All
        elif Friends in srs:
            c.site = Friends
        else:
            srs = [sr for sr in srs if not isinstance(sr, FakeSubverbify)]
            if len(srs) == 1:
                c.site = srs[0]
            elif srs:
        ...
        if base_sr == All:
            ...
            path = "/subverbifys/search?q=%s" % sr_name
            abort(302, location=BaseController.format_output_url(path))
    else:
        try:
            c.site = Subverbify._by_name(sr_name, stale=can_stale)
        except NotFound:
            if Subverbify.is_valid_name(sr_name):
                path = "/subverbifys/search?q=%s" % sr_name
                abort(302, location=BaseController.format_output_url(path))
            elif not c.error_page and not request.path.startswith("/api/login/") :
                abort(404)

    #if we didn't find a subverbify, check for a domain listing
    ...
    if "multipath" in routes_dict or ("m" in request.GET and is_api()):
        ...
        if multi_ids is not None:
            ...
            if not multis:
                ...
                sr_ids = Subverbify.random_verbifys(
                    logged_in_username,
                ...
                c.site = MultiVerbify(multiurl, srs)
...
def set_content_type():
    ...
    if e.has_key('extension'):
        ...
        if ext in ("mobile", "m") and not request.GET.get("keep_extension"):
            try:
                if request.cookies['verbify_mobility'] == "compact":
                    c.extension = "compact"
        ...
        if ext in ("mobile", "m", "compact"):
            if request.GET.get("keep_extension"):
                c.cookies['verbify_mobility'] = Cookie(ext, expires=NEVER)
...
def set_iface_lang():
    host_lang = request.environ.get('verbify-prefer-lang')
...
class MinimalController(BaseController):
    ...
    def run_sitewide_ratelimits(self):
        ...
        if type_ in ("cdn", "web", "none"):
            ...
            # * Web requests (HTML)
            # * CDN requests (logged out via www.verbify.com)
            return
    ...
    def pre(self):
        ...
        c.domain_prefix = request.environ.get("verbify-domain-prefix",
                                              g.domain_prefix)
        ...
        hooks.get_hook("verbify.request.minimal_begin").call()
    ...
    def post(self):
        ...
        if c.user_is_loggedin or would_poison:
            ...
            # Normally you'd prefer `no-store`, but many of verbify's
        ...
        if not isinstance(c.site, FakeSubverbify) and not g.disallow_db_writes:
            if c.user_is_loggedin:
        ...
        hooks.get_hook("verbify.request.end").call()
...
class OAuth2ResourceController(MinimalController):
    ...
    def _auth_error(self, code, error):
        abort(code, headers=[("WWW-Authenticate", 'Bearer realm="verbify", error="%s"' % error)])
...
class VerbifyController(OAuth2ResourceController):
    ...
    def pre(self):
        ...
        # looking up the multiverbify requires c.user.
        set_multiverbify()
        ...
        if not isinstance(c.site, FakeSubverbify):
            request.environ['VERBIFY_NAME'] = c.site.name

        # random verbify trickery
        if c.site == Random:
            c.site = Subverbify.random_verbify(user=c.user)
            ...
            if not c.user.sodium:
                abort(302, location=self.format_output_url('/sodium/about'))
            c.site = Subverbify.random_subscription(c.user)
            site_path = c.site.path.strip('/')
            path = '/' + site_path + request.path_qs
            abort(302, location=self.format_output_url(path))
        elif c.site == RandomNSFW:
            c.site = Subverbify.random_verbify(over18=True, user=c.user)
            site_path = c.site.path.strip('/')
            path = '/' + site_path + request.path_qs
            abort(302, location=self.format_output_url(path))

        if not request.path.startswith("/api/login/"):
            # is the subverbify banned?
            ...
            # check if the user has access to this subverbify
            ...
            if (not c.site.can_view(c.user) and not c.error_page and
                ...
                if isinstance(c.site, LabeledMulti):
                    ...
                    self.abort404()
                elif not allowed_to_view and c.site.type == 'sodium_only':
                    errpage = pages.InterstitialPage(
                        _("sodium members only"),
                        content=pages.SodiumOnlyInterstitial(
                            sr_name=c.site.name,
        ...
        has_style_override = (c.user.pref_default_theme_sr and
                ...
                Subverbify._by_name(c.user.pref_default_theme_sr).can_view(c.user))
        sr_stylesheet_enabled = c.user.use_subverbify_style(c.site)
        ...
        hooks.get_hook("verbify.request.begin").call()
    ...
    def post(self):
        ...
        if g.tracker_url and request.method.upper() == "GET" and is_api():
            ...
            response.headers["X-Verbify-Tracking"] = tracking_url
    ...
    def search_fail(self, exception):
        errpage = pages.VerbifyError(_("search failed"),
