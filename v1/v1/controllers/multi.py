...
from v1.controllers.verbify_base import VerbifyController, abort_with_error
from v1.controllers.oauth2 import require_oauth2_scope
from v1.models.account import Account
from v1.models.subverbify import (
    FakeSubverbify,
    Subverbify,
    LabeledMulti,
    TooManySubverbifysError,
)
...
from v1.lib.pages.things import wrap_things
...
multi_sr_data_json_spec = VValidatedJSON.Object({
    'name': VSubverbifyName('name', allow_language_srs=True),
})
...
multi_json_spec = VValidatedJSON.PartialObject({
    ...
    'subverbifys': VValidatedJSON.ArrayOf(multi_sr_data_json_spec),
})
...
class MultiApiController(VerbifyController):
    ...
    def GET_multi(self, multi, expand_srs):
        """Fetch a multi's data and subverbify list by name."""
    ...
    def _check_new_multi_path(self, path_info):
        if path_info['owner'].lower() != c.user.name.lower():
            raise VerbifyError('MULTI_CANNOT_EDIT', code=403,
                              fields='multipath')
        return c.user

    def _add_multi_srs(self, multi, sr_datas):
        srs = Subverbify._by_name(sr_data['name'] for sr_data in sr_datas)

        for sr in srs.itervalues():
            if isinstance(sr, FakeSubverbify):
                raise VerbifyError('MULTI_SPECIAL_SUBVERBIFY',
        ...
        for sr_data in sr_datas:
            try:
                ...
                raise VerbifyError('SUBVERBIFY_NOEXIST', code=400)
            else:
        ...
        except TooManySubverbifysError as e:
            raise VerbifyError('MULTI_TOO_MANY_SUBVERBIFYS', code=409)

        return sr_props

    def _write_multi_data(self, multi, data):
        srs = data.pop('subverbifys', None)
        if srs is not None:
            multi.clear_srs()
            try:
    ...
    def POST_multi(self, path_info, data):
        ...
        if not path_info:
            raise VerbifyError('BAD_MULTI_PATH', code=400)
        ...
        try:
            ...
            raise VerbifyError('MULTI_EXISTS', code=409, fields='multipath')
    ...
    def _copy_multi(self, from_multi, to_path_info, rename=False):
        ...
        if rename and from_multi.owner != to_owner:
            raise VerbifyError('MULTI_CANNOT_EDIT', code=400)
    ...
    @validate(
        ...
        to_path_info=VMultiPath("to", required=False,
            docs={"to": "destination multiverbify url path"},
        ),
        display_name=VLength("display_name", max_length=MAX_DISP_NAME,
                             empty_error=None),
    )
    ...
    def _get_multi_subverbify(self, multi, sr):
    ...
    def GET_multi_subverbify(self, multi, sr):
        """Get data about a subverbify in a multi."""
        return self._get_multi_subverbify(multi, sr)
    ...
    @validate(
        ...
        VModhash(),
        multi=VMultiByPath("multipath", require_edit=True),
        sr_name=VSubverbifyName('srname', allow_language_srs=True),
        data=VValidatedJSON("model", multi_sr_data_json_spec),
    )
    @api_doc(api_section.multis, extends=GET_multi_subverbify)
    def PUT_multi_subverbify(self, multi, sr_name, data):
        """Add a subverbify to a multi."""
    ...
    @api_doc(api_section.multis, extends=GET_multi_subverbify)
    def DELETE_multi_subverbify(self, multi, sr):
        """Remove a subverbify from a multi."""
        multi.del_srs(sr)
        multi._commit()
