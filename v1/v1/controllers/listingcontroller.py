...
from verbify_base import VerbifyController, base_listing, paginated_listing

from v1.models import *
from v1.models.query_cache import CachedQuery, MergedCachedQuery
from v1.config.extensions import is_api
from v1.lib.filters import _force_unicode
from v1.lib.jsontemplates import get_usertrophies
from v1.lib.pages import *
from v1.lib.pages.things import wrap_links
from v1.lib.menus import TimeMenu, CommentsTimeMenu, SortMenu, RecSortMenu, ProfileSortMenu
from v1.lib.menus import ControversyTimeMenu, ProfileOverviewTimeMenu, menu, QueryButton
from v1.lib.rising import get_rising, normalized_rising
from v1.lib.wrapped import Wrapped
from v1.lib.normalized_hot import normalized_hot
from v1.lib.db.thing import Query, Merge, Relations
...
from v1.lib import hooks, organic, trending
...
class ListingController(VerbifyController):
    ...
    # login box, subverbify box, submit box, etc, visible
    show_sidebar = True
    show_chooser = False
    suppress_reply_buttons = False

    # class (probably a subclass of Verbify) to use to render the page.
    render_cls = Verbify
    ...
    def keep_fn(self):
        def keep(item):
            ...
            if isinstance(c.site, AllSR):
                if not item.subverbify.discoverable:
                    return False
    ...
    def listing(self):
        ...
        if (getattr(c.site, "_id", -1) == Subverbify.get_promote_srid() and
...
class SubverbifyListingController(ListingController):
    ...
    def _build_og_title(self, max_length=256):
        ...
        # We'd like to always show the whole subverbify name, so let's
    ...
    def canonical_link(self):
        """Return the canonical link of the subverbify.

        Ordinarily canonical links are created using request.url.
        In the case of subverbifys, we perform a bit of magic to strip the
        subverbify path from the url. This means that a path like:

        https:///www.verbify.com/r/hiphopheads/

        will instead show:

        https://www.verbify.com/

        See SubverbifyMiddleware for more information.
    ...
    def render_params(self):
        ...
        if isinstance(c.site, DefaultSR):
            ...
            if not c.user_is_loggedin:
                ...
                hook = hooks.get_hook('subverbify_listing.twitter_card')
...
class ListingWithPromos(SubverbifyListingController):
    show_organic = False
    ...
    def make_spotlight(self):
        ...
        links from the user's subscribed subverbifys and promoted links targeted
        ...
        show_promo = False
        keywords = []
        can_show_promo = not c.user.pref_hide_ads or not c.user.sodium and c.site.allow_ads
    ...
    def content(self):
        ...
        if c.render_style == "html":
            spotlight = None
            show_sponsors = not c.user.pref_hide_ads or not c.user.sodium
...
class HotController(ListingWithPromos):
    ...
    def query(self):
        ...
        if isinstance(c.site, DefaultSR):
        ...
        elif isinstance(c.site, MultiVerbify):
            ...
            if sticky_fullnames:
                ...
                # the subverbify's list of sticky fullnames
    ...
    def trending_info(cls):
        ...
        trending_data = trending.get_trending_subverbifys()
        ...
        return {
            'subverbify_names': trending_data['subverbify_names'],
        ...
        if c.render_style == "html":
            ...
            if hot_pane:
                ...
                if trending_info:
                    stack = [
                        self.spotlight,
                        TrendingSubverbifysBar(**trending_info),
                        self.listing_obj,
                    ]
...
class AdsController(SubverbifyListingController):
    where = 'ads'
    ...
    def GET_listing(self, *a, **kw):
        ...
        if not c.site.allow_ads:
            self.abort404()
        return SubverbifyListingController.GET_listing(self, *a, **kw)
...
class UserController(ListingController):
    ...
    def menus(self):
        ...
        if self.where == 'saved' and c.user.sodium:
            srnames = LinkSavesBySubverbify.get_saved_subverbifys(self.vuser)
            srnames += CommentSavesBySubverbify.get_saved_subverbifys(self.vuser)
            srs = Subverbify._by_name(set(srnames), stale=True)
            ...
            if len(categories) >= 1:
                ...
                cat_menu = NavMenu(cat_buttons, base_path=base_path,
                                   ...
                                   type='lightdrop')
                res.append(cat_menu)
        elif (self.where == 'silded' and
                (c.user == self.vuser or c.user_is_admin)):
            path = '/user/%s/silded/' % self.vuser.name
            buttons = [NavButton(_("sildings received"), dest='/'),
                       NavButton(_("sildings given"), dest='/given')]
    ...
    def title(self):
        ...
        if self.where == 'silded' and self.show == 'given':
            return _("sildings given by %(user)s") % {'user': self.vuser.name}
    ...
    def keep_fn(self):
        def keep(item):
            ...
            if self.where == 'silded' and item.sildings <= 0:
                return False
    ...
    def query(self):
        ...
        if self.where == 'overview':
        ...
        elif self.where == 'silded':
            if self.show == 'given':
                q = queries.get_user_sildings(self.vuser)
            else:
                q = queries.get_silded_user(self.vuser)
            ...
            if not self.savedcategory and c.user.sodium:
    ...
    def GET_listing(self, where, vuser, sort, time, show, **env):
        ...
        if ((where in ('saved', 'hidden') or
                (where == 'silded' and show == 'given')) and
        ...
        if where == 'saved':
            ...
            if srname and c.user.sodium:
                try:
                    sr = Subverbify._by_name(srname)
                except NotFound:
                    sr = None
            else:
                sr = None
    ...
    def GET_about(self, vuser):
        """Return information about the user, including karma and sodium status."""
        if (not is_api() or
                not vuser or
                (vuser._spam and vuser != c.user)):
            return self.abort404()
        return Verbify(content = Wrapped(vuser)).render()
...
class MessageController(ListingController):
    ...
    def show_sidebar(self):
        if c.default_sr and not isinstance(c.site, (ModSR, MultiVerbify)):
            return False
    ...
    def keep_fn(self):
        def keep(item):
            ...
            if (hasattr(item, "subverbify") and
                    item.subverbify.is_moderator(c.user)):
                return wouldkeep
    ...
    def builder(self):
        if (self.where == 'messages' or
            ...
            if self.where == "multi":
                root = c.site
                message_cls = MultiverbifyMessageBuilder
                ...
                sr = self.message.subverbify_slow
            ...
            if (message_cls is UserMessageBuilder and parent and parent.sr_id
                ...
                # Make sure we use the subverbify message builder for modmail,
                # because the per-user cache will be wrong if more than two
                # parties are involved in the thread.
                root = Subverbify._byID(parent.sr_id)
                message_cls = SrMessageBuilder
    ...
    def query(self):
        if self.where == 'messages':
            ...
            q = queries.get_unread_subverbify_messages_multi(c.site.kept_sr_ids)
        elif self.where == 'moderator' and self.subwhere == 'unread':
            if c.default_sr:
                srids = Subverbify.reverse_moderator_ids(c.user)
                srs = [sr for sr in Subverbify._byID(srids, data=False,
                                                    return_dict=False)
                       if sr.is_moderator_with_perms(c.user, 'mail')]
                q = queries.get_unread_subverbify_messages_multi(srs)
            else:
                q = queries.get_unread_subverbify_messages(c.site)
    ...
    def GET_compose(self, to, subject, message):
        mod_srs = []
        subverbify_message = False
        only_as_subverbify = False
        self.where = "compose"
        ...
        if isinstance(c.site, MultiVerbify):
            ...
            if not mod_srs:
                abort(403)
            subverbify_message = True
        elif not isinstance(c.site, FakeSubverbify):
            if not c.site.is_moderator_with_perms(c.user, "mail"):
                abort(403)
            mod_srs = [c.site]
            subverbify_message = True
            only_as_subverbify = True
        elif c.user.is_moderator_somewhere:
            mod_srs = Mod.srs_with_perms(c.user, "mail")
            subverbify_message = bool(mod_srs)

        captcha = Captcha() if c.user.needs_captcha() else None

        if subverbify_message:
            content = ModeratorMessageCompose(
                mod_srs,
                only_as_subverbify=only_as_subverbify,
                to=to,
                subject=subject,
                captcha=captcha,
                message=message,
...
class VerbifysController(ListingController):
    render_cls = SubverbifysPage
    extra_page_classes = ListingController.extra_page_classes + ['subverbifys-page']

    def title(self):
        return _('subverbifys')
    ...
    def query(self):
        if self.where == 'banned' and c.user_is_admin:
            verbifys = Subverbify._query(Subverbify.c._spam == True,
                                       sort = desc('_date'),
                                       write_cache = True,
                                       read_cache = True,
                                       cache_time = 5 * 60,
            ...
            verbifys = None
            if self.where == 'new':
                verbifys = Subverbify._query( write_cache = True,
                                            read_cache = True,
                                            cache_time = 5 * 60,
                                            stale = True)
                verbifys._sort = desc('_date')
            elif self.where == 'employee':
                if c.user_is_loggedin and c.user.employee:
                    verbifys = Subverbify._query(
                        Subverbify.c.type=='employees_only',
                        write_cache=True,
                        read_cache=True,
                        cache_time=5 * 60,
                        stale=True,
                    )
                    verbifys._sort = desc('_downs')
                else:
                    abort(404)
            elif self.where == 'quarantine':
                if c.user_is_admin:
                    verbifys = Subverbify._query(
                        Subverbify.c.quarantine==True,
                        write_cache=True,
                        read_cache=True,
                        cache_time=5 * 60,
                        stale=True,
                    ...
                    abort(404)
            elif self.where == 'sodium':
                verbifys = Subverbify._query(
                    Subverbify.c.type=='sodium_only',
                    write_cache=True,
                    read_cache=True,
                    cache_time=5 * 60,
                    stale=True,
                ...
                return [
                    sr._fullname
                    for sr in Subverbify.default_subverbifys(ids=False)
                ]
            elif self.where == 'featured':
                return [
                    sr._fullname
                    for sr in Subverbify.featured_subverbifys()
                ]
            else:
                verbifys = Subverbify._query( write_cache = True,
                                            read_cache = True,
    ...
    def render_params(self):
        ...
        # event target for screenviews (/subverbifys)
        event_target = {
    ...
    @require_oauth2_scope("read")
    @listing_api_doc(section=api_section.subverbifys,
                     uri='/subverbifys/{where}',
                     uri_variants=[
                         '/subverbifys/popular',
                         '/subverbifys/new',
                         '/subverbifys/sodium',
    ...
    def GET_listing(self, where, **env):
        ...
        The `where` parameter chooses the order in which the subverbifys are
        displayed.  `popular` sorts on the activity of the subverbify and the
        position of the subverbifys can shift around. `new` sorts the subverbifys
...
class MyverbifysController(ListingController):
    render_cls = MySubverbifysPage
    extra_page_classes = ListingController.extra_page_classes + ['subverbifys-page']
    ...
    def menus(self):
        ...
        return [NavMenu(buttons, base_path = '/subverbifys/mine/',
                        default = 'subscriber', type = "flatlist")]

    def title(self):
        return _('subverbifys: ') + self.where
    ...
    def query(self):
        ...
        if self.where == "subscriber":
            sr_ids = Subverbify.subscribed_ids_by_user(c.user)
        else:
        ...
        sr_fullnames = [
            Subverbify._fullname_from_id36(to36(sr_id)) for sr_id in sr_ids]
        return sr_fullnames

    def content(self):
        user = c.user if c.user_is_loggedin else None
        num_subscriptions = len(Subverbify.subscribed_ids_by_user(user))
    ...
    def build_listing(self, after=None, **kwargs):
        if after and not isinstance(after, Subverbify):
            abort(400, 'gimme a subverbify')
    ...
    def render_params(self):
        ...
        # event target for screenviews (/subverbifys/mine)
        event_target = {
            'target_sort': self.where,
        }
        if self.after:
    ...
    @require_oauth2_scope("mysubverbifys")
    @validate(VUser())
    @listing_api_doc(section=api_section.subverbifys,
                     uri='/subverbifys/mine/{where}',
                     uri_variants=['/subverbifys/mine/subscriber', '/subverbifys/mine/contributor', '/subverbifys/mine/moderator'])
    def GET_listing(self, where='subscriber', **env):
        """Get subverbifys the user has a relationship with.

        The `where` parameter chooses which subverbifys are returned as follows:

        * `subscriber` - subverbifys the user is subscribed to
        * `contributor` - subverbifys the user is an approved submitter in
        * `moderator` - subverbifys the user is a moderator of
...
class CommentsController(SubverbifyListingController):
    ...
    def keep_fn(self):
        def keep(item):
            ...
            if c.user_is_loggedin:
                if item.subverbify.is_moderator(c.user):
                    return True
...
class UserListListingController(ListingController):
    ...
    def title(self):
        ...
        # having this suffix, to make similar tabs on different subverbifys
        # distinct.
        if self.where == 'moderators':
            return '%(section)s - /r/%(subverbify)s' % {
                'section': section_title,
                'subverbify': c.site.name,
            }
    ...
    def GET_listing(self, where, user=None, **kw):
        ...
        if where == 'contributors':
            # On public verbifys, only moderators may see the whitelist.
            if c.site.type == 'public' and not has_mod_access:
                abort(403)
            # Used for subverbifys like /r/lounge
            if c.site.hide_subscribers:
                abort(403)
            # used for subverbifys that don't allow access to approved submitters
...
class SildedController(SubverbifyListingController):
    where = 'silded'
    title_text = _("silded")
    ...
    def infotext(self):
        ...
        seconds = c.site.silding_server_seconds
        if not seconds:
            return ''
        ...
        message = _("sildings in this subverbify have paid for %(time)s of "
                    "server time")
        return message % {'time': server_time}

    @property
    def infotext_class(self):
        return "rounded sodium-accent"

    def keep_fn(self):
        def keep(item):
            return item.sildings > 0 and not item._deleted and not item._spam
        return keep

    def query(self):
        try:
            return c.site.get_silded()
        except NotImplementedError:
            abort(404)
