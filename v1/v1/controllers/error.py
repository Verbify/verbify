...
try:
    # place all v1 specific imports in here.  If there is a code error, it'll
    # get caught and the stack trace won't be presented to the user in
    # production
    from v1.config import extensions
    from v1.controllers.verbify_base import VerbifyController, UnloggedUser
    from v1.lib.cookies import Cookies
    from v1.lib.errors import ErrorSet
    ...
    from v1.lib import log, pages
    from v1.lib.strings import get_funny_translated_string
    from v1.lib.template_helpers import static
    from v1.lib.base import abort
    from v1.models.link import Link
    from v1.models.subverbify import DefaultSR, Subverbify
except Exception, e:
    if g.debug:
...
'''<html>
  <head>
    <title>verbify broke!</title>
  </head>
  <body>
    <div style="margin: auto; text-align: center">
      <p>
        <a href="/">
          <img border="0" src="%s" alt="you broke verbify" />
        </a>
      </p>
      <p>
        %s
...
class ErrorController(VerbifyController):
    ...
    # List of admins to blame (skip the first admin, "verbify")
    ...
    def __after__(self): 
        try:
            VerbifyController.__after__(self)
        except Exception as e:
            handle_awful_failure("ErrorController.__after__: %r" % e)

    def __call__(self, environ, start_response):
        try:
            return VerbifyController.__call__(self, environ, start_response)
        except Exception as e:
    ...
    def send404(self):
        if 'usable_error_content' in request.environ:
            return request.environ['usable_error_content']
        return pages.VerbifyError(_("page not found"),
    ...
    def GET_document(self):
        try:
            ...
            if srname:
                c.site = Subverbify._by_name(srname)
            ...
            if code in (204, 304):
                ...
                return verbifybroke % (failien_url, sad_message)
            elif code == 503:
                return self.send503()
            elif c.site:
                return self.send404()
...
def handle_awful_failure(fail_text):
    ...
    try:
        ...
        for line in traceback.format_exc().splitlines():
            g.log.error(line)
        return verbifybroke % (make_failien_url(), websafe(fail_text))
    except:
