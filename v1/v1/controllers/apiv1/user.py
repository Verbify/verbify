...
from v1.lib.pages import FriendTableItem
...
from v1.models import Account, Trophy
import v1.lib.errors as errors
import v1.lib.validator.preferences as vprefs
...
class APIv1UserController(OAuth2OnlyController):
    ...
    def GET_karma(self):
        """Return a breakdown of subverbify karma."""
    ...
    def PUT_friends(self, friend, notes_json):
        ...
        if 'name' in notes_json and notes_json['name'] != friend:
            ...
            err = errors.VerbifyError('BAD_USERNAME', fields='name')
        if 'note' in notes_json and not c.user.sodium:
            err = errors.VerbifyError('SODIUM_REQUIRED', fields='note')
        if err:
