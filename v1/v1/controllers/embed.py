...
from v1.lib.base import proxyurl
from v1.lib.csrf import csrf_exempt
from v1.lib.template_helpers import get_domain
from v1.lib.pages import Embed, BoringPage, HelpPage
from v1.lib.filters import websafe, SC_OFF, SC_ON
...
def renderurl_cached(path):
    # Needed so http://verbify.com/help/ works
    fp = path.rstrip("/")
    u = "https://code.verbify.com/wiki" + fp + '?stripped=1'
...
class EmbedController(VerbifyController):
    ...
    def GET_blog(self):
        return self.redirect("https://blog.%s/" %
                             get_domain(subverbify=False, no_www=True))
