...
from verbify_base import VerbifyController, MinimalController, require_https
from v1.lib.db import tdb_cassandra
from v1.lib.db.thing import NotFound
from v1.lib.pages import VerbifyError
from v1.lib.strings import strings
from v1.models import Account, admintools, create_gift_sodium, send_system_message
from v1.models.token import (
    OAuth2Client, OAuth2AuthorizationCode, OAuth2AccessToken,
    OAuth2RefreshToken, OAuth2Scope)
from v1.lib.errors import BadRequestError, ForbiddenError, errors
from v1.lib.pages import OAuth2AuthorizationPage
from v1.lib.require import RequirementException, require, require_split
from v1.lib.utils import constant_time_compare, parse_http_basic, UrlParser
...
class OAuth2FrontendController(VerbifyController):
    ...
    def POST_authorize(self, authorize, client, redirect_uri, scope, state,
        ...
        # mobile app, sild them
        if (g.live_config.get('mobile_sild_first_login') and
                not c.user.has_used_mobile_app and
                client._id in g.mobile_auth_sild_clients):
            buyer = Account.system_user()
            admintools.adjust_sodium_expiration(
                c.user, days=g.mobile_auth_sild_time)
            create_gift_sodium(
                buyer._id, c.user._id, g.mobile_auth_sild_time,
                datetime.now(g.tz), signed=True, note='first_mobile_auth')
            subject = 'Let there be sodium! Verbify just sent you Verbify sodium!'
            message = (
                "Thank you for using the Verbify mobile app!  As a thank you "
