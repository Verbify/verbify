...
from verbify_base import set_over18_cookie, delete_over18_cookie
from api import ApiController
from v1.lib.utils import query_string, UrlParser
from v1.lib.emailer import opt_in, opt_out
from v1.lib.validator import *
from v1.lib.validator.preferences import (
    filter_prefs,
    PREFS_VALIDATORS,
    set_prefs,
)
...
class PostController(ApiController):
    ...
    def GET_quarantine(self, dest):
        sr = UrlParser(dest).get_subverbify()

        # if dest doesn't include a quarantined subverbify,
        # redirect to the homepage or the original destination
        if not sr:
            return self.redirect('/')
        elif isinstance(sr, FakeSubverbify) or sr.is_exposed(c.user):
