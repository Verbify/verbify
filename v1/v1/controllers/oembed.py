...
from v1.lib.filters import scriptsafe_dumps, websafe, _force_unicode
from v1.lib.utils import url_to_thing, UrlParser
from v1.lib.template_helpers import format_html, make_url_https
from v1.lib.validator import can_view_link_comments, validate, VBoolean, VUrl
from v1.models import Comment, Link, Subverbify

_OEMBED_BASE = {
    "version": "1.0",
    "provider_name": "verbify",
    "provider_url": make_url_https('/'),
}

EMBEDLY_SCRIPT = 'https://embed.verbifymedia.com/widgets/platform.js'
SCRIPT_TEMPLATE = '<script async src="%(embedly_script)s" charset="UTF-8"></script>'
POST_EMBED_TEMPLATE = """
    <blockquote class="verbify-card" %(live_data_attr)s>
      <a href="%(link_url)s">%(title)s</a> from
      <a href="%(subverbify_url)s">%(subverbify_name)s</a>
    </blockquote>
    %(script)s
"""
...
def _oembed_post(thing, **embed_options):
    ...
    if (not can_view_link_comments(thing) or
            subverbify.type in Subverbify.private_types):
    ...
    html = format_html(POST_EMBED_TEMPLATE,
                       ...
                       subverbify_url=make_url_https(subverbify.path),
                       subverbify_name=subverbify.name,
                       script=script,
                       )
