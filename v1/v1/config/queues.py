...
def declare_queues(g):
    queues = Queues({
        ...
        "subverbify_query_q": MessageQueue(bind_to_self=True),
    ...
    if g.shard_subverbify_query_queues:
        sharded_subverbify_query_queues = {
            "subverbify_query_%d_q" % i: MessageQueue(bind_to_self=True)
            for i in xrange(10)
        }
        queues.declare(sharded_subverbify_query_queues)
