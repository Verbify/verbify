...
def not_in_sr(environ, results):
    return ('subverbify' not in environ and
...
def make_map(config):
    ...
    for plugin in reversed(config['v1.plugins']):
    ...
    ABOUT_BASE = "https://about.verbify.com/"
    ...
    mc('/traffic/subverbifys/report', controller='front',
       action='subverbify_traffic_report')
    mc('/account-activity', controller='front', action='account_activity')

    mc('/subverbifys/create', controller='front', action='newverbify')
    mc('/subverbifys/search', controller='front', action='search_verbifys')
    mc('/subverbifys/login', controller='forms', action='login')
    mc('/subverbifys/:where', controller='verbifys', action='listing',
       where='popular', conditions={'function':not_in_sr},
       requirements=dict(where="popular|new|banned|employee|sodium|default|"
                               "quarantine|featured"))
    # If no subverbify is specified, might as well show a list of 'em.
    mc('/r', controller='redirect', action='redirect', dest='/subverbifys')

    mc('/subverbifys/mine/:where', controller='myverbifys', action='listing',
    ...
    # Using the above /subverbifys/ ones instead is preferable
    mc('/verbifys/create', controller='front', action='newverbify')
    mc('/verbifys/search', controller='front', action='search_verbifys')
    mc('/verbifys/login', controller='forms', action='login')
    mc('/verbifys/:where', controller='verbifys', action='listing',
       where='popular', conditions={'function':not_in_sr},
       requirements=dict(where="popular|new|banned"))

    mc('/verbifys/mine/:where', controller='myverbifys', action='listing',
       where='subscriber', conditions={'function':not_in_sr},
       requirements=dict(where='subscriber|contributor|moderator'))
    ...
    mc('/i18n', controller='redirect', action='redirect',
       dest='https://www.verbify.com/r/i18n')
    ...
    mc('/admin/cverbifys', controller='admintool', action='cverbifys')
    mc('/admin/sodium', controller='admintool', action='sodium')
    ...
    for connect in (mc,) + multi_prefixes:
       ...
       connect('/about/:location', controller='front', action='editverbify',
          requirements=dict(location='edit|stylesheet|traffic|about'))
       connect('/comments', controller='comments', action='listing')
       connect('/comments/silded', action='listing', controller='silded')
       connect('/silded', action='listing', controller='silded')
    ...
    # preserve timeverbify URLs from 4/1/2012
    mc('/t/:timeverbify', controller='redirect', action='timeverbify_redirect')
    mc('/t/:timeverbify/*rest', controller='redirect',
       action='timeverbify_redirect')
    ...
    mc('/sodium', controller='forms', action="sodium", is_payment=False)
    mc('/sodium/payment', controller='forms', action="sodium", is_payment=True)
    mc('/sodium/creditsild/:passthrough', controller='forms', action='creditsild')
    mc('/sodium/thanks', controller='front', action='sodiumthanks')
    mc('/sodium/subscription', controller='forms', action='subscription')
    mc('/silding', controller='front', action='silding')
    mc('/cverbifys', controller='redirect', action='redirect', 
       dest='/sodium?sodiumtype=cverbifys')
    ...
    mc('/help/sodium', controller='redirect', action='redirect',
       dest='/sodium/about')
    ...
    mc('/faq', controller='redirect', action='redirect',
       dest='https://verbify.zendesk.com/')
    ...
    mc('/api/spendcverbifys', controller='ipn', action="spendcverbifys")
    mc('/api/stripecharge/sodium', controller='stripe', action='sodiumcharge')
    mc('/api/modify_subscription', controller='stripe',
       action='modify_subscription')
    mc('/api/cancel_subscription', controller='stripe',
       action='cancel_subscription')
    mc('/api/stripewebhook/sodium/:secret', controller='stripe',
       action='sodiumwebhook')
    mc('/api/coinbasewebhook/sodium/:secret', controller='coinbase',
       action='sodiumwebhook')
    mc('/api/rgwebhook/sodium/:secret', controller='verbifygifts',
       action='sodiumwebhook')
    ...
    mc('/api/recommend/sr/:srnames', controller='api',
       action='subverbify_recommendations')
    ...
    mc("/api/multi/*multipath/r/:srname", controller="multiapi", action="multi_subverbify")
    ...
    mc("/api/filter/*multipath/r/:srname", controller="multiapi", action="multi_subverbify")
    ...
    mc("/api/v1/sodium/sild/:fullname", controller="apiv1sodium", action="sild")
    mc("/api/v1/sodium/give/:username", controller="apiv1sodium", action="give")
    ...
    # verbify.com/http://... and verbify.com/34fr
