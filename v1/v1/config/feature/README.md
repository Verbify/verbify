...
`v1.config.feature` is verbify's feature flagging API. It lets us quickly
...
# On when viewing certain subverbifys
feature_some_flag = {"subverbifys": ["wtf", "aww"]}
...
feature_some_flag = {"admin": true, "users": ["user1", "usev1"]}
```
...
# this will enable the flag for sodium users, and then run an experiment for other logged in users
feature_some_flag = {"sodium": true, "experiment": {"loggedin": true, "experiment_id": 12345, "variants": {"test_something": 5.5, "test_something_else": 10, "control_1": 20, "control_2": 20}}}
```
