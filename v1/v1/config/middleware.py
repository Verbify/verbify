...
from v1.config import hooks
from v1.config.environment import load_environment
from v1.config.extensions import extension_mapping, set_extension
from v1.lib.utils import is_subdomain, is_language_subdomain
from v1.lib import csrf, filters
...
def error_mapper(code, message, environ, global_conf=None, **kw):
    ...
    if code in codes:
        ...
        exception = environ.get('v1.controller.exception')
        if exception:
        ...
        if environ.get('VERBIFY_NAME'):
            d['srname'] = environ.get('VERBIFY_NAME')
        if environ.get('VERBIFY_TAKEDOWN'):
            d['takedown'] = environ.get('VERBIFY_TAKEDOWN')
        if environ.get('VERBIFY_ERROR_NAME'):
            d['error_name'] = environ.get('VERBIFY_ERROR_NAME')
...
class DomainMiddleware(object):
    ...
    def __call__(self, environ, start_response):
        ...
        # tell verbify_base to redirect to the appropriate subverbify for
        # a legacy CNAME
        ...
        for subdomain in subdomains[:]:
            ...
            if subdomain in g.reserved_subdomains:
                ...
                environ['verbify-domain-extension'] = extension
            elif is_language_subdomain(subdomain):
                environ['verbify-prefer-lang'] = subdomain
            else:
                sr_redirect = subdomain
                subdomains.remove(subdomain)

        if 'verbify-prefer-lang' in environ:
            prefix_parts.insert(0, environ['verbify-prefer-lang'])
        if prefix_parts:
            environ['verbify-domain-prefix'] = '.'.join(prefix_parts)

        # if there was a subverbify subdomain, redirect
...
class SubverbifyMiddleware(object):
    ...
    def __call__(self, environ, start_response):
        ...
        if sr:
            environ['subverbify'] = sr.groups()[0]
...
class DomainListingMiddleware(object):
    ...
    def __call__(self, environ, start_response):
        if not environ.has_key('subverbify'):
            path = environ['PATH_INFO']
...
class ExtensionMiddleware(object):
    ...
    def __call__(self, environ, start_response):
        ...
        domain_ext = environ.get('verbify-domain-extension')
...
class VerbifyApp(PylonsApp):

    test_mode = False

    def __init__(self, *args, **kwargs):
        super(VerbifyApp, self).__init__(*args, **kwargs)
    ...
    def _check_csrf_prevention(self):
        ...
        if not g.running_as_script:
            controllers_iter = itertools.chain(
                controllers._verbify_controllers.itervalues(),
    ...
    def load_controllers(self):
        ...
        self.config['v1.plugins'].load_controllers()
...
def make_app(global_conf, full_stack=True, **app_conf):
    ...
    if config['v1.plugins'] and g.config['uncompressedJS']:
        plugin_static_apps = Cascade([StaticURLParser(plugin.static_dir)
                                      for plugin in config['v1.plugins']])
