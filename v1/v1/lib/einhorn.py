...
To run a paste-based application (like v1) under Einhorn using Gunicorn as the
...
isn't managing workers.
    ...
    einhorn -n 4 -b 0.0.0.0:8080 python -m v1.lib.einhorn example.ini
...
class EinhornSyncWorker(SyncWorker):
    ...
    def init_signals(self):
        ...
        # einhorn will send SIGUSV1 to request a graceful shutdown
        signal.signal(signal.SIGUSV1, self.start_graceful_shutdown)
        signal.siginterrupt(signal.SIGUSV1, False)
