...
class VerbifyDetectorBase(DetectorBase):
    agent_string = None
    version_string = '(\.?\d+)*'
    ...
    def detect(self, agent, result):
        detected = super(VerbifyDetectorBase, self).detect(agent, result)
...
class VerbifyBrowser(VerbifyDetectorBase, Browser):
    """Base class for all verbify specific browsers."""
...
@register_detector
class VerbifyIsFunDetector(VerbifyBrowser):
    is_app = True
    look_for = 'verbify is fun'
    name = 'verbify is fun'
...
@register_detector
class VerbifyAndroidDetector(VerbifyBrowser):
    is_app = True
    look_for = 'VerbifyAndroid'
    name = 'Verbify: The Official App'
    agent_string = '{look_for} (?P<version>{version_string})$'


@register_detector
class VerbifyIOSDetector(VerbifyBrowser):
    is_app = True
    look_for = 'Verbify'
    name = 'verbify iOS'
...
@register_detector
class AlienBlueDetector(VerbifyBrowser):
    is_app = True
    look_for = 'AlienBlue'
    name = 'Alien Blue'
    agent_string = (
...
@register_detector
class RelayForVerbifyDetector(VerbifyBrowser):
    is_app = True
    look_for = 'Relay by /u/DBrady'
    name = 'relay for verbify'
    agent_string = '{look_for} v(?P<version>{version_string})'


@register_detector
class VerbifySyncDetector(VerbifyBrowser):
    is_app = True
    look_for = 'verbify_sync'
    name = 'Sync for verbify'
    agent_string = (
...
@register_detector
class NarwhalForVerbifyDetector(VerbifyBrowser):
    is_app = True
    look_for = 'narwhal'
    name = 'narwhal for verbify'
    agent_string = '{look_for}-(?P<platform>.*?)\/\d+ by det0ur'


@register_detector
class McVerbifyDetector(VerbifyBrowser):
    is_app = True
    look_for = 'McVerbify'
    name = 'McVerbify'
    agent_string = '{look_for} - Verbify Client for (?P<platform>.*?)$'


@register_detector
class ReaditDetector(VerbifyBrowser):
    look_for = 'Readit'
    name = 'Readit'
...
@register_detector
class BaconReaderDetector(VerbifyBrowser):
    is_app = True
