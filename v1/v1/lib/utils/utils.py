...
import snudown
import unidecode
from v1.lib.utils import verbify_agent_parser
...
from v1.lib.contrib import ipaddress
...
from v1.lib.utils._utils import *
...
class UrlParser(object):
    ...
    The class also contains verbify-specific functions for setting,
    checking, and getting a path's subverbify.
    """
    ...
    def switch_subdomain_by_extension(self, extension=None):
        ...
        This should only be used on verbify URLs.
        ...
        * extension: the template extension to which the middleware hints when
        ...
        >>> u.unparse()
        'http://i.verbify.com/r/verbifydev'
    ...
    def path_has_subverbify(self):
        """
        utility method for checking if the path starts with a
        subverbify specifier (namely /r/ or /subverbifys/).
        """
        return self.path.startswith(('/r/', '/subverbifys/', '/verbifys/'))

    def get_subverbify(self):
        """checks if the current url refers to a subverbify and returns
        that subverbify object.  The cases here are:

          * the hostname is unset or is g.domain, in which case it
            looks for /r/XXXX or /subverbifys.  The default in this case
            is Default.
          * the hostname is a cname to a known subverbify.

        On failure to find a subverbify, returns None.
        """
        from v1.models import Subverbify, NotFound, DefaultSR
        try:
            if (not self.hostname or
                    is_subdomain(self.hostname, g.domain) or
                    self.hostname.startswith(g.domain)):
                if self.path.startswith('/r/'):
                    return Subverbify._by_name(self.path.split('/')[2])
                else:
                    return DefaultSR()
            elif self.hostname:
                return Subverbify._by_domain(self.hostname)
        except NotFound:
            pass
        return None
    ...
    def is_web_safe_url(self):
        ...
        # Credentials in the netloc? Not on verbify!
        if "@" in self._orig_netloc:
            return False

        # `javascript://www.verbify.com/%0D%Aalert(1)` is not safe, obviously
    ...
    def is_verbify_url(self, subverbify=None):
        """utility method for seeing if the url is associated with
        verbify as we don't necessarily want to mangle non-verbify
        domains

        returns true only if hostname is nonexistant, a subdomain of
        g.domain, or a subdomain of the provided subverbify's cname.
        """

        valid_subdomain = (
            not self.hostname or
            is_subdomain(self.hostname, g.domain) or
            (subverbify and subverbify.domain and
                is_subdomain(self.hostname, subverbify.domain))
        )
    ...
    def path_add_subverbify(self, subverbify):
        """
        Adds the subverbify's path to the path if another subverbify's
        prefix is not already present.
        """
        if not (self.path_has_subverbify()
                or self.path.startswith(subverbify.user_path)):
            self.path = (subverbify.user_path + self.path)
        return self
    ...
    def domain_permutations(self, fragments=False, subdomains=True):
        """
          Takes a domain like `www.verbify.com`, and returns a list of ways
...
def url_to_thing(url):
    """Given a verbify URL, return the Thing to which it associates.
    ...
    """
    from v1.models import Comment, Link, Message, NotFound, Subverbify, Thing
    from v1.config.middleware import SubverbifyMiddleware
    sr_pattern = SubverbifyMiddleware.sr_pattern
    ...
    if not path or path == '/':
        ...
        try:
            return Subverbify._by_name(sr_name, data=True)
        except NotFound:
            return None
...
def decompose_fullname(fullname):
    """
        decompose_fullname("t3_e4fa") ->
            (Thing, 3, 658918)
    """
    from v1.lib.db.thing import Thing,Relation
    if fullname[0] == 't':
        type_class = Thing
    elif fullname[0] == 'r':
        type_class = Relation
...
def fix_if_broken(thing, delete = True, fudge_links = False):
    from v1.models import Link, Comment, Subverbify, Message

    # the minimum set of attributes that are required
    attrs = dict((cls, cls._essentials)
                 for cls
                 in (Link, Comment, Subverbify, Message))
...
def find_recent_broken_things(from_time = None, to_time = None,
    ...
    """
        ...
        them as appropriate.
    """
    from v1.models import Link, Comment
...
def url_links_builder(url, exclude=None, num=None, after=None, reverse=None,
    ...
    from v1.models import IDBuilder, Link, NotFound, Subverbify
    ...
    if public_srs_only and not c.user_is_admin:
        subverbifys = Subverbify._byID([link.sr_id for link in links], data=True)
        links = [link for link in links
                 if subverbifys[link.sr_id].type not in Subverbify.private_types]
        ...
        return (not link._spam or
                (c.user_is_loggedin and
                    (link.author_id == c.user._id or
                        c.user_is_admin or
                        link.subverbify.is_moderator(c.user))))
...
def config_sodium_price(v, key=None, data=None):
    return SodiumPrice(v)
