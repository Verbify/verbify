...
v1's existing diagnostics infrastructure. It is not meant to be the last word
on v1+baseplate; ideally v1 will move towards using more of baseplate rather
...
class V1BaseplateObserver(BaseplateObserver):
    def on_server_span_created(self, context, server_span):
        observer = V1ServerSpanObserver()
        server_span.register(observer)


class V1ServerSpanObserver(ServerSpanObserver):
    def on_child_span_created(self, span):
        observer = V1SpanObserver(span.name)
        span.register(observer)


class V1SpanObserver(SpanObserver):
