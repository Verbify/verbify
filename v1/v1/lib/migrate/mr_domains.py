...
"""
Generate the data for the listings for the time-based Subverbify
queries. The format is eventually that of the CachedResults objects
used by v1.lib.db.queries (with some intermediate steps), so changes
...
cd ~/verbify/v1
time psql -F"\t" -A -t -d newverbify -U $USER -h $LINKDBHOST \
     -c "\\copy (select t.thing_id, 'thing', 'link',
                        t.ups, t.downs, t.deleted, t.spam, extract(epoch from t.date)
                   from verbify_thing_link t
                  where not t.spam and not t.deleted
                  )
                  to 'verbify_thing_link.dump'"
time psql -F"\t" -A -t -d newverbify -U $USER -h $LINKDBHOST \
     -c "\\copy (select d.thing_id, 'data', 'link',
                        d.key, d.value
                   from verbify_data_link d
                  where d.key = 'url' ) to 'verbify_data_link.dump'"
cat verbify_data_link.dump verbify_thing_link.dump | sort -T. -S200m | paster --plugin=v1 run $INI v1/lib/migrate/mr_domains.py -c "join_links()" > links.joined
cat links.joined | paster --plugin=v1 run $INI v1/lib/migrate/mr_domains.py -c "time_listings()" | sort -T. -S200m | paster --plugin=v1 run $INI v1/lib/migrate/mr_domains.py -c "write_permacache()"
"""

import sys

from v1.models import Account, Subverbify, Link
