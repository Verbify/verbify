...
## links
time psql -F"\t" -A -t -d newverbify -U ri -h $LINKDBHOST \
     -c "\\copy (select t.thing_id, 'thing', 'link',
                        t.ups, t.downs, t.deleted, t.spam, extract(epoch from t.date)
                   from verbify_thing_link t) to 'verbify_thing_link.dump'"
time psql -F"\t" -A -t -d newverbify -U ri -h $LINKDBHOST \
     -c "\\copy (select d.thing_id, 'data', 'link',
                        d.key, d.value
                   from verbify_data_link d
                  where d.key = 'author_id' or d.key = 'sr_id') to 'verbify_data_link.dump'"
pv verbify_data_link.dump verbify_thing_link.dump | sort -T. -S200m | ./mr_permacache "join_links()" > links.joined
pv links.joined | ./mr_permacache "link_listings()" | sort -T. -S200m > links.listings

## comments
psql -F"\t" -A -t -d newverbify -U ri -h $COMMENTDBHOST \
     -c "\\copy (select t.thing_id, 'thing', 'comment',
                        t.ups, t.downs, t.deleted, t.spam, extract(epoch from t.date)
                   from verbify_thing_comment t) to 'verbify_thing_comment.dump'"
psql -F"\t" -A -t -d newverbify -U ri -h $COMMENTDBHOST \
     -c "\\copy (select d.thing_id, 'data', 'comment',
                        d.key, d.value
                   from verbify_data_comment d
                  where d.key = 'author_id') to 'verbify_data_comment.dump'"
cat verbify_data_comment.dump verbify_thing_comment.dump | sort -T. -S200m | ./mr_permacache "join_comments()" > comments.joined
cat links.joined | ./mr_permacache "comment_listings()" | sort -T. -S200m > comments.listings

## linkvotes
psql -F"\t" -A -t -d newverbify -U ri -h $VOTEDBHOST \
     -c "\\copy (select r.rel_id, 'vote_account_link',
                        r.thing1_id, r.thing2_id, r.name, extract(epoch from r.date)
                   from verbify_rel_vote_account_link r) to 'verbify_linkvote.dump'"
pv verbify_linkvote.dump | ./mr_permacache "linkvote_listings()" | sort -T. -S200m > linkvotes.listings

#savehide
psql -F"\t" -A -t -d newverbify -U ri -h $SAVEHIDEDBHOST \
     -c "\\copy (select r.rel_id, 'savehide',
                        r.thing1_id, r.thing2_id, r.name, extract(epoch from r.date)
                   from verbify_rel_savehide r) to 'verbify_savehide.dump'"
pv verbify_savehide.dump | ./mr_permacache "savehide_listings()" | sort -T. -S200m > savehide.listings
...
from v1.lib import mr_tools
from v1.lib.mr_tools import dataspec_m_thing, dataspec_m_rel, join_things


from dateutil.parser import parse as parse_timestamp

from v1.models import *
from v1.lib.db.sorts import epoch_seconds, score, controversy, _hot
from v1.lib.utils import fetch_things2, in_chunks, progress, UniqueIterator, tup
from v1.lib import comment_tree
from v1.lib.db import queries

from v1.lib.jsontemplates import make_fullname # what a strange place
                                               # for this function
...
def store_keys(key, maxes):
    ...
    # individual lookups for accounts and subverbifys
