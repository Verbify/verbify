...
"""
One-time use functions to migrate from one verbify-version to another
"""
...
def recompute_unread(min_date = None):
    from v1.models import Inbox, Account, Comment, Message
...
def pushup_permacache(verbosity=1000):
    ...
    def gen_keys():
        ...
        sr_q = Subverbify._query(Subverbify.c._spam == (True, False),
                                sort=desc('_date'),
                                )
...
def convert_query_cache_to_json():
    import cPickle
    from v1.models.query_cache import json, UserQueryCache
