...
from v1.config.extensions import api_type, is_api
from v1.lib import utils, captcha, promote, totp, ratelimit
from v1.lib.filters import unkeep_space, websafe, _force_unicode, _force_utf8
from v1.lib.filters import markdown_souptest
from v1.lib.db import tdb_cassandra
from v1.lib.db.operators import asc, desc
from v1.lib.souptest import (
    SoupError,
    SoupDetectedCrasherError,
    SoupUnsupportedEntityError,
)
from v1.lib.template_helpers import add_sr
from v1.lib.jsonresponse import JQueryResponse, JsonResponse
from v1.lib.permissions import ModeratorPermissionSet
from v1.models import *
from v1.models.rules import MAX_RULES_PER_SUBVERBIFY
from v1.models.promo import Location
from v1.lib.authorize import Address, CreditCard
from v1.lib.utils import constant_time_compare
from v1.lib.require import require, require_split, RequirementException
from v1.lib import signing

from v1.lib.errors import errors, VerbifyError, UserRequiredException
from v1.lib.errors import VerifiedUserRequiredException
...
def can_view_link_comments(article):
    return (article.subverbify_slow.can_view(c.user) and
...
class VMarkdown(Validator):
    def __init__(self, param, renderer='verbify'):
...
class VMarkdownLength(VMarkdown):
    def __init__(self, param, renderer='verbify', max_length=10000,
...
class VSubverbifyName(VRequired):
    ...
    def run(self, name):
        ...
        valid_name = Subverbify.is_valid_name(
    ...
    def param_docs(self):
        return {
            self.param: "subverbify name",
        }


class VAvailableSubverbifyName(VSubverbifyName):
    def run(self, name):
        name = VSubverbifyName.run(self, name)
        if name:
            try:
                a = Subverbify._by_name(name)
                return self.error(errors.SUBVERBIFY_EXISTS)
            except NotFound:
                return name
...
class VSRByName(Validator):
    ...
    def run(self, sr_name):
        if not sr_name:
            ...
            try:
                sr = Subverbify._by_name(sr_name)
                if self.return_srname:
                    return sr.name
                else:
                    return sr
            except NotFound:
                self.set_error(errors.SUBVERBIFY_NOEXIST, code=400)
...
class VSRByNames(Validator):
    """Returns a dict mapping subverbify names to subverbify objects.

    sr_names_csv - a comma delimited string of subverbify names
    required - if true (default) an empty subverbify name list is an error
    ...
    def run(self, sr_names_csv):
        if sr_names_csv:
            ...
            return Subverbify._by_name(sr_names)
        elif self.required:
            self.set_error(errors.BAD_SR_NAME, code=400)
        return {}
    ...
    def param_docs(self):
        return {
            self.param: "comma-delimited list of subverbify names",
        }


class VSubverbifyTitle(Validator):
    def run(self, title):
        if not title:
...
class VSubverbifyDesc(Validator):
...
class VAvailableSubverbifyRuleName(Validator):
    ...
    def run(self, short_name):
        ...
        if SubverbifyRules.get_rule(c.site, short_name):
            self.set_error(errors.SR_RULE_EXISTS)
        elif not self.updating:
            number_rules = len(SubverbifyRules.get_rules(c.site))
            if number_rules >= MAX_RULES_PER_SUBVERBIFY:
                self.set_error(errors.SR_RULE_TOO_MANY)
                return None
        return short_name


class VSubverbifyRule(Validator):
    def run(self, short_name):
        short_name = VLength(
            self.param,
            max_length=50,
        ...
        rule = SubverbifyRules.get_rule(c.site, short_name)
        if not rule:
            self.set_error(errors.SR_RULE_DOESNT_EXIST)
        else:
            return rule
...
class VAccountByName(VRequired):
    ...
    def param_docs(self):
        return {self.param: "A valid, existing verbify username"}
...
class VNotInTimeout(Validator):
    def run(self, target_fullname=None, fatal=True, action_name=None,
            details_text="", target=None, subverbify=None):
        if c.user_is_loggedin and c.user.in_timeout:
            g.events.timeout_forbidden_event(
                ...
                subverbify=subverbify,
                request=request,
                context=c,
            )
            if fatal:
                request.environ['VERBIFY_ERROR_NAME'] = 'IN_TIMEOUT'
...
class VSodium(VUser):
    notes = "*Requires a subscription to [verbify sodium](/sodium/about)*"
    def run(self):
        VUser.run(self)
        ...
        if c.user_is_loggedin:
            ...
            if item.author_id == c.user._id:
                if isinstance(item, Message) and c.user.employee:
                    return True
                subverbify = item.subverbify_slow

                if (how in ("yes", "no") and
                        subverbify.can_distinguish(c.user)):
                ...
                if can_distinguish:
                    ...
                    VNotInTimeout().run(target=item, subverbify=subverbify)
    ...
    def run(self, thing_name):
        if c.user_is_admin:
            ...
            if item.author_id == c.user._id:
                ...
                # comments
                subverbify = item.subverbify_slow
                if subverbify.can_distinguish(c.user):
                    can_alter = True
...
class VSrSpecial(VByName):
    def run(self, thing_name):
        if c.user_is_admin:
            ...
            # comments
            subverbify = item.subverbify_slow
            if subverbify.is_special(c.user):
                return True
        abort(403,'forbidden')
...
class VSubmitSR(Validator):
    ...
    def run(self, sr_name, link_type = None):
        if not sr_name:
            self.set_error(errors.SUBVERBIFY_REQUIRED)
            return None
        ...
        try:
            ...
            self.set_error(errors.SUBVERBIFY_NOEXIST)
            return

        if not c.user_is_loggedin or not sr.can_submit(c.user, self.promotion):
            self.set_error(errors.SUBVERBIFY_NOTALLOWED)
            return

        if not sr.allow_ads and self.promotion:
            self.set_error(errors.SUBVERBIFY_DISABLED_ADS)
            return
    ...
    def param_docs(self):
        return {
            self.param[0]: "name of a subverbify",
        }
...
class VSubscribeSR(VByName):
    ...
    def run(self, sr_id, sr_name):
        ...
        try:
            sr = Subverbify._by_name(str(sr_name).strip())
        except (NotFound, AttributeError, UnicodeEncodeError):
            self.set_error(errors.SUBVERBIFY_NOEXIST)
            return
    ...
    def param_docs(self):
        return {
            self.param[0]: "the name of a subverbify",
        }
...
class VMessageRecipient(VExistingUname):
    def run(self, name):
        ...
        if (c.user.in_timeout and
                not (is_subverbify and
                     '/r/%s' % name == g.admin_message_acct)):
            abort(403, 'forbidden')

        if is_subverbify:
            try:
                s = Subverbify._by_name(name)
                if isinstance(s, FakeSubverbify):
                    raise NotFound, "fake subverbify"
                if s._spam:
                    raise NotFound, "banned subverbify"
...
class VCssName(Validator):
    ...
    def param_docs(self):
        return {
            self.param: "a valid subverbify image name",
        }
...
class VDestination(Validator):
    ...
    def run(self, dest):
        ...
        if ld.startswith(('/', 'http://', 'https://')):
            ...
            if u.is_verbify_url(c.site) and u.is_web_safe_url():
                return dest
...
class VFlairTemplateByID(VRequired):
    ...
    def run(self, flair_template_id):
        try:
            return FlairTemplateBySubverbifyIndex.get_template(
    ...
    class ArrayOf(object):
        ...
        def run(self, data):
            if not isinstance(data, list):
                raise VerbifyError('JSON_INVALID', code=400)
        ...
        def run(self, data, ignore_missing=False):
            ...
            for key, validator in self.spec.iteritems():
                try:
                    ...
                    if ignore_missing:
                        continue
                    raise VerbifyError('JSON_MISSING_KEY', code=400,
                                      msg_params={'key': key})
            return validated_data
...
class VMultiPath(Validator):
    """Validates a multiverbify path. Returns a path info dictionary.
    """
    ...
    def param_docs(self):
        return {
            self.param: "multiverbify url path",
        }


class VMultiByPath(Validator):
    """Validates a multiverbify path.  Returns a LabeledMulti.
    """
...
class VSubverbifyList(Validator):
    ...
    def run(self, subverbifys):
        if not subverbifys:
            return []

        # extract subverbify name if path provided
        subverbifys = [sr_path_rx.sub('\g<name>', sr.strip())
                      for sr in subverbifys.lower().strip().splitlines() if sr]
        ...
        unique_srs = set(subverbifys)

        if subverbifys:
            valid_srs = set(Subverbify._by_name(subverbifys).keys())
            if unique_srs - valid_srs:
                return self.set_error(errors.SUBVERBIFY_NOEXIST, code=400)

        if len(unique_srs) > self.limit:
            return self.set_error(
                errors.TOO_MANY_SUBVERBIFYS, {'max': self.limit}, code=400)

        # return list of subverbify names as entered
        return subverbifys

    def param_docs(self):
        return {
            self.param: 'a list of subverbify names, line break delimited',
        }
...
class VSigned(Validator):
    ...
    :py:function:`~v1.lib.signing.valid_ua_signature` and in the case
    of POST and PUT ensure that any request.body included is also signed
    via :py:function:`~v1.lib.signing.valid_body_signature`.
