...
from v1.lib.validator.validator import (
    VBoolean,
    VInt,
    VLang,
    VOneOf,
...
from v1.models import Subverbify, NotFound
...
PREFS_VALIDATORS = dict(
    ...
    pref_media=VOneOf('media', ('on', 'off', 'subverbify')),
    # pref_media_preview=VOneOf('media_preview', ('on', 'off', 'subverbify')),
    ...
    pref_show_sodium_expiration=VBoolean("show_sodium_expiration"),
    ...
    pref_cverbify_autorenew=VBoolean("cverbify_autorenew"),
...
def set_prefs(user, prefs):
    for k, v in prefs.iteritems():
        if k == 'pref_beta' and v and not getattr(user, 'pref_beta', False):
            ...
            # to the beta subverbify.
            try:
                sr = Subverbify._by_name(g.beta_sr)
                if not sr.is_subscriber(user):
                    sr.add_subscriber(user)
            except NotFound:
                g.log.warning("Could not find beta subverbify '%s'. It may "
