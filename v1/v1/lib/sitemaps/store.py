...
This module is uploads all subverbify sitemaps as well as the sitemap index
...
store_sitemaps_in_s3(subverbifys)

Even though the subverbifys are expected to be generated and passed into this
...
If we simply queried the subverbify iterator for it's length then we would
have to load all of the subverbifys into memory, which would be ... bad.
"""
...
from v1.lib.sitemaps.generate import subverbify_sitemaps, sitemap_index
...
def store_subverbify_sitemap(bucket, index, sitemap):
    key = Key(bucket)
    key.key = 'subverbify_sitemap/{0}.xml'.format(index)
...
def store_sitemap_index(bucket, count):
    key = Key(bucket)
    key.key = g.sitemap_subverbify_keyname
    g.log.debug("Uploading %r", key)

    upload_sitemap(key, sitemap_index(count))


def store_sitemaps_in_s3(subverbifys):
    s3conn = S3Connection()
    bucket = s3conn.get_bucket(g.sitemap_upload_s3_bucket, validate=False)

    sitemap_count = 0
    for i, sitemap in enumerate(subverbify_sitemaps(subverbifys)):
        store_subverbify_sitemap(bucket, i, sitemap)
        sitemap_count += 1
