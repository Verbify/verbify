...
from v1.lib.s3_helpers import parse_s3_path
from v1.lib.sitemaps.store import store_sitemaps_in_s3
from v1.lib.sitemaps.data import find_all_subverbifys
...
def _subverbify_sitemap_key():
    conn = S3Connection()
    bucket = conn.get_bucket(g.sitemap_upload_s3_bucket, validate=False)
    return bucket.get_key(g.sitemap_subverbify_keyname)
...
def _before_last_sitemap(timestamp):
    sitemap_key = _subverbify_sitemap_key()
...
def _process_message():
    ...
    subverbifys = find_all_subverbifys(s3path)
...
def _create_test_message():
    ...
    message = sqs_q.new_message(body=json.dumps({
        ...
        'location': ('s3://verbify-data-analysis/big-data/v1/prod/' +
