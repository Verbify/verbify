...
from v1.lib import amqp, hooks
from v1.lib.language import charset_summary
from v1.lib.geoip import (
    get_request_location,
    location_by_ips,
)
from v1.lib.cache_poisoning import cache_headers_valid
...
class EventQueue(object):
    ...
    def vote_event(self, vote):
        ...
        vote: An v1.models.vote Vote object
        """
        ...
        event.add_subverbify_fields(vote.thing.subverbify_slow)
    ...
    def submit_event(self, new_post, request=None, context=None):
        ...
        new_post: An v1.models.Link object
        ...
        event.add_subverbify_fields(new_post.subverbify_slow)
    ...
    def comment_event(self, new_comment, request=None, context=None):
        ...
        new_comment: An v1.models.Comment object
        request, context: Should be pylons.request & pylons.c respectively
        """
        ...
        event.add_subverbify_fields(new_comment.subverbify_slow)
    ...
    @squelch_exceptions
    def muted_forbidden_event(self, details_text, subverbify=None,
        ...
        details_text: "muted" if a muted user is trying to message the
            subverbify or "muted mod" if the subverbify mod is attempting
            to message the muted user
        subverbify: The Subverbify of the mod messaging the muted user
        parent_message: Message that is being responded to
        target: The intended recipient (Subverbify or Account)
        request, context: Should be pylons.request & pylons.c respectively;
        ...
        event.add_subverbify_fields(subverbify)
    ...
    def timeout_forbidden_event(self, action_name, details_text,
            target=None, target_fullname=None, subverbify=None,
        ...
        subverbify: The Subverbify the action was taken in. If target is of the
            type Subverbify, then this won't be passed in
        request, context: Should be pylons.request & pylons.c respectively;
        ...
        if not action_name:
            ...
            # set or unset for contest mode and subverbify sticky
            elif action_name in ("set_contest_mode", "set_subverbify_sticky"):
        ...
        from v1.models import Comment, Link, Subverbify
        if not subverbify:
            if isinstance(context.site, Subverbify):
                subverbify = context.site
            elif isinstance(target, (Comment, Link)):
                subverbify = target.subverbify_slow
            elif isinstance(target, Subverbify):
                subverbify = target
    ...
    def mod_event(self, modaction, subverbify, mod, target=None,
            request=None, context=None):
        """Create a 'mod' event for event-collector.

        modaction: An v1.models.ModAction object
        subverbify: The Subverbify the mod action is being performed in
    ...
    def report_event(self, reason=None, details_text=None,
            subverbify=None, target=None, request=None, context=None,
        ...
        subverbify: The Subverbify the action is being performed in
        target: The Thing the action was applied to
        request, context: Should be pylons.request & pylons.c respectively

        """
        from v1.models.rules import OLD_SITEWIDE_RULES, SITEWIDE_RULES, SubverbifyRules
        ...
        if reason in OLD_SITEWIDE_RULES or reason in SITEWIDE_RULES:
            ...
            if subverbify and SubverbifyRules.get_rule(subverbify, reason):
                process_notes = "SUBVERBIFY_RULES"
            else:
    ...
    def quarantine_event(self, event_type, subverbify,
            request=None, context=None):
        ...
        subverbify: The quarantined subverbify
        request, context: Should be pylons.request & pylons.c respectively
...
class Event(object):
    ...
    def add_subverbify_fields(self, subverbify):
        if not subverbify:
            return

        self.add("sr_id", subverbify._id)
        self.add("sr_name", subverbify.name)
