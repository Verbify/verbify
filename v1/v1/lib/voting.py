...
from v1.lib.eventcollector import Event
from v1.lib.utils import epoch_timestamp, is_subdomain, UrlParser
from v1.models import Account, Comment, Link, Subverbify
from v1.models.last_modified import LastModified
from v1.models.query_cache import CachedQueryMutator
from v1.models.vote import Vote, VotesByAccount

from v1.lib.geoip import organization_by_ips
...
def update_user_liked(vote):
    from v1.lib.db.queries import get_disliked, get_liked
...
def consume_author_query_queue(qname="author_query_q", limit=1000):
    ...
    def process_message(msgs, chan):
        ...
        from v1.lib.db.queries import add_queries, get_submitted
...
def consume_subverbify_query_queue(qname="subverbify_query_q", limit=1000):
    @g.stats.amqp_processor(qname)
    def process_message(msgs, chan):
...
def consume_comment_vote_queue(qname="vote_comment_q"):
    @g.stats.amqp_processor(qname)
    def process_message(msg):
        from v1.lib.comment_tree import write_comment_scores
        from v1.lib.db.queries import (
            add_queries,
            add_to_commentstree_q,
            get_comments,
        )
        from v1.models.builder import get_active_sort_orders_for_link
