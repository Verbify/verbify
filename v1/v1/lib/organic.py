...
from v1.lib import count
from v1.lib.utils import UniqueIterator, timeago
...
def cached_organic_links(*sr_ids):
    ...
    #only use links from verbifys that you're subscribed to
...
def organic_links(user):
    ...
    # get the default subverbifys if the user is not logged in
    user_id = None if isinstance(user, FakeAccount) else user
    sr_ids = Subverbify.user_subverbifys(user, True)
