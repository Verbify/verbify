...
from v1.models import Comment, Email, DefaultSR, Account, Award
from v1.models.token import EmailVerificationToken, PasswordResetToken
...
def _system_email(email, plaintext_body, kind, reply_to="",
    ...
    feedback and the name will be verbify.com)
    """
...
def verify_email(user, dest=None):
    ...
    """
    from v1.lib.pages import VerifyEmail
...
def password_email(user):
    ...
    """
    from v1.lib.pages import PasswordReset
...
def password_change_email(user):
    ...
    from v1.lib.pages import PasswordChangeEmail
...
def email_change_email(user):
    ...
    from v1.lib.pages import EmailChangeEmail
...
def send_queued_mail(test = False):
    ...
    sent_mail list."""
    from v1.lib.pages import Share, Mail_Opt
    now = datetime.datetime.now(g.tz)
    if not c.site:
        c.site = DefaultSR()
...
def _promo_email(thing, kind, body = "", **kw):
    from v1.lib.pages import Promo_Email
...
def suspicious_payment(user, link):
    from v1.lib.pages import SuspiciousPaymentEmail
