...
def get_system_from_address(sr):
    return "r/{subverbify} mail <{sender_email}>".format(
        subverbify=sr.name, sender_email=g.modmail_system_email)
...
def send_modmail_email(message):
    ...
    if message.from_sr and not message.first_message:
        ...
        sender_text = ("This message was sent from r/{subverbify} to "
            "u/{user}").format(subverbify=sr.name, user=recipient.name)
    else:
