...
from v1.lib.emr_helpers import (EmrException, terminate_jobflow,
    modify_slave_count)
from v1.lib.s3_helpers import get_text_from_s3, s3_key_exists, copy_to_s3
from v1.lib.traffic.emr_traffic import (extract_hour, aggregate_interval,
        coalesce_interval)
from v1.lib.utils import tup
from v1.models.traffic import (SitewidePageviews, PageviewsBySubverbify,
        PageviewsBySubverbifyAndPath, PageviewsByLanguage,
...
traffic_categories = (SitewidePageviews, PageviewsBySubverbify,
                      PageviewsBySubverbifyAndPath, PageviewsByLanguage,
                      ClickthroughsByCodename, TargetedClickthroughsByCodename,
                      AdImpressionsByCodename, TargetedImpressionsByCodename)
...
traffic_subdirectories = {
    ...
    PageviewsBySubverbify: 'subverbify',
    PageviewsBySubverbifyAndPath: 'srpath',
...
def _name_to_kw(category_cls, name):
    ...
    def target_split(name):
        ...
        Name is (link,campaign-subverbify) where link and campaign are
        thing fullnames. campaign and subverbify are each optional, so
        ...
        if not target_info:
            ...
            campaign_codename, subverbify = target_info.split('-', 1)
        elif target_info.find('_') != -1:
            campaign_codename = target_info
            subverbify = ''
        else:
            subverbify = target_info
        return {'codename': campaign_codename or link_codename,
                'subverbify': subverbify}

    d = {SitewidePageviews: lambda n: {},
         PageviewsBySubverbify: lambda n: {'subverbify': n},
         PageviewsBySubverbifyAndPath: lambda n: {'srpath': n},
...
def _report_interval(interval):
    ...
    from v1.models.traffic import engine
