...
###############################################################################
"""View models for the traffic statistic pages on verbify."""
...
from v1.lib.menus import NavButton, NamedButton, PageNameNav, NavMenu
from v1.lib.pages.pages import Verbify, TimeSeriesChart, TabbedPane
from v1.lib.promote import cost_per_mille, cost_per_click
from v1.lib.template_helpers import format_number
from v1.lib.utils import Storage, to_date, timedelta_by_name
from v1.lib.wrapped import Templated
from v1.models import Thing, Link, PromoCampaign, traffic
from v1.models.subverbify import Subverbify, _DefaultSR
...
class TrafficPage(Verbify):
    ...
    def __init__(self, content):
        Verbify.__init__(self, title=_("traffic stats"), content=content)
...
class VerbifyTraffic(Templated):
...
def make_subverbify_traffic_report(subverbifys=None, num=None):
    """Return a report of subverbify traffic in the last full month.

    If given a list of subverbifys, those subverbifys will be put in the report
    otherwise the top subverbifys by pageviews will be automatically chosen.

    """

    if subverbifys:
        subverbify_summary = traffic.PageviewsBySubverbify.last_month(subverbifys)
    else:
        subverbify_summary = traffic.PageviewsBySubverbify.top_last_month(num)

    report = []
    for srname, data in subverbify_summary:
        if srname == _DefaultSR.name:
            name = _("[frontpage]")
            url = None
        elif srname in Subverbify._specials:
            name = "[%s]" % srname
            url = None
        else:
            name = "/r/%s" % srname
...
class SitewideTraffic(VerbifyTraffic):
    """An overview of all traffic to the site."""
    def __init__(self):
        self.subverbify_summary = make_subverbify_traffic_report(num=250)
        VerbifyTraffic.__init__(self, g.domain)
...
class AdvertTrafficSummary(VerbifyTraffic):
    """An overview of traffic for all adverts on the site."""

    def __init__(self):
        VerbifyTraffic.__init__(self, _("adverts"))
    ...
    def get_sr_name(name):
        """Return the display name for a subverbify."""
...
class LanguageTraffic(VerbifyTraffic):
    def __init__(self, langcode):
        self.langcode = langcode
        name = LanguageTraffic.get_language_name(langcode)
        VerbifyTraffic.__init__(self, name)
...
class AdvertTraffic(VerbifyTraffic):
    def __init__(self, code):
        self.code = code
...
class SubverbifyTraffic(VerbifyTraffic):
    def __init__(self):
        VerbifyTraffic.__init__(self, "/r/" + c.site.name)
    ...
    def get_dow_summary(self):
        return traffic.PageviewsBySubverbify.history("day", c.site.name)

    def get_data_for_interval(self, interval, columns):
        pageviews = traffic.PageviewsBySubverbify.history(interval, c.site.name)
        ...
        if interval == "day":
            ...
            sr_name = c.site.name
            subscriptions = traffic.SubscriptionsBySubverbify.history(interval,
                                                                     sr_name)
...
class SubverbifyTrafficReport(Templated):
    def __init__(self):
