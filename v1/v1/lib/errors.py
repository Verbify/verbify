...
from v1.lib.utils import Storage, tup
...
error_list = dict((
        ...
        ('SUBVERBIFY_EXISTS', _('that subverbify already exists')),
        ('SUBVERBIFY_NOEXIST', _('that subverbify doesn\'t exist')),
        ('SUBVERBIFY_NOTALLOWED', _("you aren't allowed to post there.")),
        ('SUBVERBIFY_NO_ACCESS', _("you aren't allowed access to this subverbify")),
        ('SUBVERBIFY_REQUIRED', _('you must specify a subverbify')),
        ('SUBVERBIFY_DISABLED_ADS', _('this subverbify has chosen to disable their ads at this time')),
        ...
        ('SUBVERBIFY_RATELIMIT', _("you are doing that too much. try again later.")),
        ...
        ('OVERSOLD', _('that subverbify has already been oversold on %(start)s to %(end)s. Please pick another subverbify or date.')),
        ...
        ('NO_SELFS', _("that subverbify doesn't allow text posts")),
        ('NO_LINKS', _("that subverbify only allows text posts")),
        ...
        ('DOMAIN_BANNED', _('%(domain)s is not allowed on verbify: %(reason)s')),
        ...
        ('NO_INVITE_FOUND', _('there is no pending invite for that subverbify')),
        ...
        ('MULTI_NOT_FOUND', _('that multiverbify doesn\'t exist')),
        ('MULTI_EXISTS', _('that multiverbify already exists')),
        ('MULTI_CANNOT_EDIT', _('you can\'t change that multiverbify')),
        ('MULTI_TOO_MANY_SUBVERBIFYS', _('no more space for subverbifys in that multiverbify')),
        ('MULTI_SPECIAL_SUBVERBIFY', _("can't add special subverbify %(path)s")),
        ('TOO_MANY_SUBVERBIFYS', _('maximum %(max)s subverbifys')),
        ...
        ('BANNED_FROM_SUBVERBIFY', _('that user is banned from the subverbify')),
        ('IN_TIMEOUT', _("You can't do that while suspended.")),
        ('SODIUM_REQUIRED', _('you must have an active verbify sodium subscription to do that')),
        ('INSUFFICIENT_CVERBIFYS', _("insufficient cverbifys")),
        ('SILDING_NOT_ALLOWED', _("silding is not allowed in this subverbify")),
        ('SCRAPER_ERROR', _("unable to scrape provided url")),
        ('NO_SR_TO_SR_MESSAGE', _("can't send a message from a subverbify to another subverbify")),
        ('USER_BLOCKED_MESSAGE', _("can't send message to that user")),
        ('ADMIN_REQUIRED', _("you must be in admin mode for this")),
        ('CANT_CONVERT_TO_SODIUM_ONLY', _("to convert an existing subverbify to sodium only, send a message to %(admin_modmail)s") 
            % dict(admin_modmail=g.admin_message_acct)),
        ('SODIUM_ONLY_SR_REQUIRED', _("this subverbify must be 'sodium only' to select this")),
        ('CANT_CREATE_SR', _("your account is too new or you do not have enough karma to create a subverbify. please contact the admins to request an exemption.")),
        ...
        ('USER_MUTED', _("You have been muted from this subverbify.")),
        ('MUTED_FROM_SUBVERBIFY', _("This user has been muted from the subverbify.")),
        ('COMMENT_NOT_STICKYABLE', _("This comment is not stickyable. Ensure that it is a top level comment.")),
        ('SR_RULE_EXISTS', _("A subverbify rule by that name already exists.")),
        ('SR_RULE_DOESNT_EXIST', _("No subverbify rule by that name exists.")),
        ('SR_RULE_TOO_MANY', _("This subverbify already has the maximum number of rules.")),
...
class VerbifyError(Exception):
    ...
    def __repr__(self):
        return '<VerbifyError: %s>' % self.name
...
class ErrorSet(object):
    ...
    def add(self, error_name, msg_params=None, field=None, code=None):
        for field_name in tup(field):
            e = VerbifyError(error_name, msg_params, fields=field_name,
                            code=code)
            self.add_error(e)
...
def verbify_http_error(code=400, error_name='UNKNOWN_ERROR', **data):
    exc = status_map[code]()
...
class UserRequiredException(VerbifyError):
    name = errors.USER_REQUIRED
    code = 403


class VerifiedUserRequiredException(VerbifyError):
