...
from v1.lib.cache_poisoning import make_poisoning_report_mac
from v1.lib.utils import UrlParser, timeago, timesince, is_subdomain

from v1.lib import hooks
from v1.lib.static import static_mtime
from v1.lib import js, tracking
...
def class_dict():
    t_cls = [Link, Comment, Message, Subverbify]
...
def get_domain(cname=False, subverbify=True, no_www=False):
    """
    returns the domain on the current subverbify, possibly including
    the subverbify part of the path, suitable for insertion after an
     ...
     * subverbify: flags whether or not to append to the domain the
       subverbify path (without the trailing path).
...
def add_sr(
    ...
    parses the path and updates it to include the subverbify path
    ...
    u = UrlParser(path)
    if sr_path:
        u.path_add_subverbify(c.site)

    if not u.hostname or force_hostname:
        u.hostname = get_domain(subverbify=False)

    if (c.secure and u.is_verbify_url()) or force_https:
        u.scheme = "https"
...
def add_attr(attrs, kind, label=None, link=None, cssclass=None, symbol=None):
    ...
    if kind == 'F':
        ...
        cssclass = 'admin'
        if not label:
            label = _('verbify admin, speaking officially')
    elif kind in ('X', '@'):
        priority = 5
        cssclass = 'gray'
        if not label:
...
def add_moderator_distinguish(distinguish_attribs_list, subverbify):
    link = '/r/%s/about/moderators' % subverbify.name
    label = _('moderator of /r/%(verbify)s, speaking officially')
    label %= {'verbify': subverbify.name}
...
def add_cakeday_distinguish(distinguish_attribs_list, user):
    label = _("%(user)s just celebrated a verbify birthday!")
...
def add_submitter_distinguish(distinguish_attribs_list, link, subverbify):
    permalink = link.make_permalink(subverbify)
    add_attr(distinguish_attribs_list, 'S', link=permalink)


def search_url(query, subverbify, restrict_sr="off", sort=None, recent=None, ref=None):
    import urllib
    ...
    if recent:
        url_query["t"] = recent
    path = "/r/%s/search?" % subverbify if subverbify else "/search?"
