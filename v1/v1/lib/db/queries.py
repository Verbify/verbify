...
from v1.lib import filters
from v1.lib.comment_tree import add_comments
from v1.lib.db import tdb_cassandra
from v1.lib.db.operators import and_, or_
from v1.lib.db.operators import asc, desc, timeago
from v1.lib.db.sorts import epoch_seconds
from v1.lib.db.thing import Thing, Merge
from v1.lib import utils
from v1.lib.utils import in_chunks, is_subdomain, SimpleSillyStub
from v1.lib.utils import fetch_things2, tup, UniqueIterator
from v1.lib.voting import prequeued_vote_key
...
from v1.models.promo import PROMOTE_STATUS, PromotionLog
from v1.models.query_cache import (
    cached_query,
    CachedQuery,
    CachedQueryMutator,
    filter_thing,
...
class CachedResults(object):
    ...
    def can_insert(self):
        """True if a new item can just be inserted rather than
         ...
         # the top hottest items in X some subverbify where the query
...
def _get_links(sr_id, sort, time):
    """General link query for a subverbify."""
...
@cached_query(SubverbifyQueryCache)
...
def moderated_srids(sr, user):
    if isinstance(sr, (ModContribSR, MultiVerbify)):
        srs = Subverbify._byID(sr.sr_ids, return_dict=False)
        if user:
...
def get_domain_links(domain, sort, time):
    from v1.lib.db import operators
...
def _get_sr_comments(sr_id):
    """the subverbify /r/foo/comments page"""
...
cached_srrel_query = cached_query(SubverbifyQueryCache, filter_thing2)
...
@cached_srrel_query
def get_subverbify_messages(sr):
    return rel_query(ModeratorInbox, sr, 'inbox')

@cached_srrel_query
def get_unread_subverbify_messages(sr):
    return rel_query(ModeratorInbox, sr, 'inbox',
                          filters = [ModeratorInbox.c.new == True])

def get_unread_subverbify_messages_multi(srs):
    if not srs:
        return []
    queries = [get_unread_subverbify_messages(sr) for sr in srs]
    return MergedCachedQuery(queries)
...
def _promoted_link_query(user_id, status):
    ...
    q = Link._query(Link.c.sr_id == Subverbify.get_promote_srid(),
                    Link.c._spam == (True, False),
...
@cached_query(SubverbifyQueryCache, filter_fn=filter_thing)
def get_all_silded_comments():
    return FakeQuery(sort=[desc("date")])


@cached_query(SubverbifyQueryCache, filter_fn=filter_thing)
def get_all_silded_links():
    return FakeQuery(sort=[desc("date")])


@merged_cached_query
def get_all_silded():
    return [get_all_silded_comments(), get_all_silded_links()]


@cached_query(SubverbifyQueryCache, filter_fn=filter_thing)
def get_silded_comments(sr_id):
    return FakeQuery(sort=[desc("date")])


@cached_query(SubverbifyQueryCache, filter_fn=filter_thing)
def get_silded_links(sr_id):
    return FakeQuery(sort=[desc("date")])


@merged_cached_query
def get_silded(sr_ids):
    queries = [get_silded_links, get_silded_comments]
    return [query(sr_id)
            for sr_id, query in itertools.product(tup(sr_ids), queries)]


@cached_query(UserQueryCache, filter_fn=filter_thing)
def get_silded_user_comments(user_id):
    return FakeQuery(sort=[desc("date")])


@cached_query(UserQueryCache, filter_fn=filter_thing)
def get_silded_user_links(user_id):
    return FakeQuery(sort=[desc("date")])


@merged_cached_query
def get_silded_users(user_ids):
    queries = [get_silded_user_links, get_silded_user_comments]
    return [query(user_id)
            for user_id, query in itertools.product(tup(user_ids), queries)]


@cached_query(UserQueryCache, filter_fn=filter_thing)
def get_user_sildings(user_id):
    return FakeQuery(sort=[desc("date")])


@merged_cached_query
def get_silded_user(user):
    return [get_silded_user_comments(user), get_silded_user_links(user)]
...
def new_link(link):
    ...
    sr = Subverbify._byID(link.sr_id)
...
def new_comment(comment, inbox_rels):
    ...
    sr = Subverbify._byID(comment.sr_id)
    ...
    # v1.lib.db.queries.run_new_comments (to minimise lock contention)

    if inbox_rels:
        update_comment_notifications(comment, inbox_rels)


def new_subverbify(sr):
    "no precomputed queries here yet"
    amqp.add_item('new_subverbify', sr._fullname)


def new_message(message, inbox_rels, add_to_sent=True, update_modmail=True):
    from v1.lib.comment_tree import add_message
    ...
    with CachedQueryMutator() as m:
        ...
        for inbox_rel in tup(inbox_rels):
            ...
            if isinstance(inbox_rel, ModeratorInbox):
                m.insert(get_subverbify_messages(to), [inbox_rel])
    ...
    if update_modmail:
        mod_perms = message.subverbify_slow.moderators_with_perms()
...
def update_unread_sr_queries(inbox_rels, insert=True, mutator=None):
    ...
    for inbox_rel in inbox_rels:
        ...
        query = get_unread_subverbify_messages(sr)
...
def unread_handler(things, user, unread):
    ...
    comments = []
    messages = []
    # Group things by subverbify or type
    for thing in things:
    ...
    if sr_messages:
        mod_srs = Subverbify.reverse_moderator_ids(user)
        srs = Subverbify._byID(sr_messages.keys())
    else:
        mod_srs = []
...
def _by_srid(things, srs=True):
    """Takes a list of things and returns them in a dict separated by
       sr_id, in addition to the looked-up subverbifys"""
    ret = {}
    ...
    if srs:
        _srs = Subverbify._byID(ret.keys(), return_dict=True) if ret else {}
        return ret, _srs
    else:
        return ret
...
def ban(things, filtered=True):
    ...
    for sr_id, sr_things in by_srid.iteritems():
        ...
        for item in sr_things:
            # don't add posts by banned users if subverbify prefs exclude them
            add_to_modqueue = (filtered and
                       not (item.subverbify_slow.exclude_banned_modqueue and
...
def new_report(thing, report_rel):
    ...
    # report queue as well as the per-subverbify one
...
def add_all_srs():
    """Recalculates every listing query for every subverbify. Very,
       very slow."""
...
def consume_deleted_accounts():
    ...
    def process_deleted_accounts(msg):
        ...
        if account.has_stripe_subscription:
            from v1.controllers.ipn import cancel_stripe_subscription
            cancel_stripe_subscription(account.sodium_subscr_id)
