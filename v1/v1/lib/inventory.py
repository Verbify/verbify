...
from v1.lib.inventory_optimization import get_maximized_pageviews
from v1.lib.memoize import memoize
from v1.lib.utils import to_date, tup
...
from v1.models.promo_metrics import LocationPromoMetrics, PromoMetrics
from v1.models.subverbify import DefaultSR
...
def _min_daily_pageviews_by_sr(ndays=NDAYS_TO_QUERY, end_date=None):
    ...
    cls = traffic.PageviewsBySubverbifyAndPath
...
def get_available_pageviews(targets, start, end, location=None, datestr=False,
    ...
    A location is: subverbify > country > metro
    ...
    * /r/funny + USA + Boston (all campaigns targeting /r/funny and USA and
    ...
    target_srs = list(chain.from_iterable(
        target.subverbifys_slow for target in targets))
    all_campaigns = find_campaigns(target_srs, start, end, ignore)

    # get predicted pageviews for each subverbify and location
    all_sr_names = set(sr.name for sr in target_srs)
    all_sr_names |= set(chain.from_iterable(
        campaign.target.subverbify_names for campaign in all_campaigns
    ))
    all_srs = Subverbify._by_name(all_sr_names).values()
    ...
    for campaign in all_campaigns:
        ...
        sr_names = tuple(sorted(campaign.target.subverbify_names))
    ...
    for target in targets:
        ...
        subverbify_names = target.subverbify_names
        ret[name] = {}
        for date in dates:
            ...
            for location in locations:
                ...
                pageviews_by_location[location] = get_maximized_pageviews(
                    subverbify_names, booked_by_target, pageviews_by_sr_name)
