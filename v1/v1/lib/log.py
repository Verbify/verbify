...
def get_operational_exceptions():
    ...
    return (
        ...
        v1.lib.db.thing.NotFound,
        v1.lib.lock.TimeoutExpired,
...
class RavenErrorReporter(Reporter):
    ...
    @classmethod
    def add_verbify_context(cls, client):
        verbify_context = {
            "language": c.lang,
        ...
        if c.site:
            verbify_context["subverbify"] = c.site.name

        client.extra_context(verbify_context)
    ...
    def get_raven_client(cls):
        app_path_prefixes = [
            "v1",
            "verbify_",  # plugins such as 'verbify_liveupdate'
        ...
        RAVEN_CLIENT = raven.Client(
            ...
            processors=[
                ...
                'v1.lib.log.SanitizeStackLocalsProcessor',
            ],
    ...
    def capture_exception(cls, exc_info=None):
        ...
        if g.running_as_script:
            ...
            cls.add_verbify_context(client)
